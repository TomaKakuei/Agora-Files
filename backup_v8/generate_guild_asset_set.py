...
def _load_map_renderer():
    ...
    if str(package_root) not in sys.path:
        sys.path.insert(0, str(package_root))
    from asset_pipeline.render_structured_map_asset import render_component_icon, render_map_asset
...
def _write_current_alias(package_root: Path, manifest_path: Path) -> None:
    ...
    alias_paths = [
        ...
        package_root / "frontend" / "assets" / "generated" / "guild_asset_sets" / "current_guild_pixel_set.json",
    ]
...
def main() -> None:
    ...
    map_source_path = batch_root / "guild_map_source.png"
    map_config = pipeline.get("map_generation", {})
    map_adapter = str(map_config.get("adapter", "room_part_compositor"))
    
    if map_adapter in {"vertex_sdk_image", "flux_local_service"} and not args.skip_map_prompt and map_prompt_result.get("status") == "ok":
        print(f"[MAP_PIPELINE] Generating AI Map using adapter: {map_adapter}", flush=True)
        try:
            map_render_result = _generate_ai_map(
                prompt=map_prompt_result["text"],
                world_config=world_config,
                pipeline=pipeline,
                output_path=map_source_path,
                batch_root=batch_root,
            )
        except Exception as ai_map_error:
            print(f"[MAP_PIPELINE] AI Map generation failed: {ai_map_error}. Falling back to structured renderer.", flush=True)
    ...
    def run_agent_generation(index: int, agent_id: str) -> tuple[int, dict[str, Any], dict[str, Any] | None]:
        ...
        if result.returncode == 0 and asset_bundle.is_file():
            ...
            bundle_publishable = (
                ...
                and (
                    str(agent_record["asset_bundle"].get("overall_status", "")).strip().lower() == "pass"
                    or sprite_status == "quality_warning_retained_source"
                )
                and reused_status in {"", "not_used", "ok"}
                and sprite_status not in {"quality_fallback_procedural", "fallback"}
                and sprite_source != "procedural_demo_sheet"
            )
    ...
    compatibility_manifest = package_root / "frontend" / "assets" / "generated" / "guild_asset_sets" / args.revision / "guild_asset_set_manifest.json"
