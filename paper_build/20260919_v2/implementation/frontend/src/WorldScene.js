...
export class WorldScene extends Phaser.Scene {
  ...
  agentDisplayScaleFor() {
    ...
    // Legacy packages used 24px agents on 32px tiles, which made otherwise
    // valid atlases nearly illegible. Keep larger authored sizes, with a
    // visible-body-aware display floor for older worlds. Map render_scale enlarges the
    // tile coordinate system, so it must enlarge character sprites as well.
    // The configured value scales the 32px atlas frame, not its non-transparent
    // body. Current compiled bodies occupy about 88% of the frame, so a 64px
    // frame keeps the visible figure near 1.75 logical tiles tall.
    const configuredSize = Number(mapVisual.agent_sprite_size || 64);
    const targetSpriteSize = Math.max(64, Number.isFinite(configuredSize) ? configuredSize : 64);
    const renderScale = Math.max(1, Number(this.displayMetrics?.renderScale || mapVisual.render_scale || 1));
    return Phaser.Math.Clamp((targetSpriteSize * renderScale) / 32, 0.95, 8);
  }
