...
export class ActionController {
  ...
  chooseCounterRequestedItem(inventory, excludeItemId = "") {
    const currencyItemId = String(this.worldConfig?.economy?.currency_item_id || "");
    return safeArray(inventory)
      .filter((entry) => {
        ...
        return (
          ...
          && itemMeta.tradable !== false
        );
      })
      .sort((left, right) => {
        const valueDelta = Number(this.itemCatalog.get(right.item_id)?.price || 0)
          - Number(this.itemCatalog.get(left.item_id)?.price || 0);
        return valueDelta || String(left.item_id).localeCompare(String(right.item_id));
      })[0]?.item_id || "";
  }
