...
export class WorldRenderer {
  ...
  drawWorld() {
    const cleanVisualQaCapture = this.captureMode === "visual_qa";
    ...
    safeArray(this.mapGrid.rooms).forEach((room) => {
      ...
      roomRect.on("pointerover", () => {
        if (cleanVisualQaCapture) {
          return;
        }
        roomNode.hovered = true;
        this.applyRoomHighlight();
      });
      roomRect.on("pointerout", () => {
        if (cleanVisualQaCapture) {
          return;
        }
        roomNode.hovered = false;
        this.applyRoomHighlight();
      });
      this.roomNodes.set(room.room_id, roomNode);
    });
  }
  ...
  applyRoomHighlight() {
    ...
    this.roomNodes.forEach((node, roomId) => {
      ...
      const revealLabel = !cleanVisualQaCapture && (active || node.hovered);
      node.title.setAlpha(revealLabel ? 1 : 0);
      node.footer.setAlpha(revealLabel ? 0.92 : 0);
    });
