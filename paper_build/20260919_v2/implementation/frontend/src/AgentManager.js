...
export class AgentManager {
  ...
  #controlRingStyleFor(agentRecord, selected = false) {
    const agentId = String(agentRecord?.agent_id || "").trim();
    const claimedBySessionId = String(agentRecord?.claimed_by_session_id || "").trim();
    const controlMode = String(agentRecord?.control_mode || "").trim().toLowerCase();
    const localSessionId = String(this.scene?.liveState?.sessionId || this.scene?.liveState?.session?.session_id || "").trim();
    const localClaimedAgentId = String(this.scene?.liveState?.session?.claimed_agent_id || "").trim();
    const locallyControlled = Boolean(
      agentId
      && (
        agentId === localClaimedAgentId
        || (claimedBySessionId && localSessionId && claimedBySessionId === localSessionId)
      )
    );
    const humanControlled = Boolean(
      claimedBySessionId
      || controlMode === "human"
      || controlMode === "player"
      || controlMode === "live_human"
    );
    if (locallyControlled) {
      return {
        fill: 0x78d6d6,
        fillAlpha: selected ? 0.32 : 0.24,
        stroke: 0xf0b25b,
        strokeAlpha: 1,
  ...
  #applySelectionVisual(sprite, agentRecord) {
    ...
    const emphasis = selected ? 1.16 : 1;
    // Legacy atlases often preserve a realistic full-body silhouette so
    // aggressively that the reduced sprite is only 6-8 pixels wide. A small
    // horizontal readability correction keeps those packages usable while new
    // assets are rejected by the minimum-width QA gate.
    sprite.setScale(baseScale * emphasis * 1.28, baseScale * emphasis);
    ...
    if (indicator) {
      const ringStyle = this.#controlRingStyleFor(agentRecord, selected);
      indicator.setPosition(sprite.x, sprite.y + 16);
      indicator.setFillStyle(ringStyle.fill, ringStyle.fillAlpha);
      indicator.setStrokeStyle(ringStyle.strokeWidth, ringStyle.stroke, ringStyle.strokeAlpha);
      indicator.setScale(selected ? 1.12 : 1);
  ...
  registerAgent(agentRecord, x, y) {
    ...
    const ringStyle = this.#controlRingStyleFor(agentRecord, false);
    const indicator = this.scene.add.ellipse(x, y + 16, 34, 16, ringStyle.fill, ringStyle.fillAlpha);
    indicator.setStrokeStyle(ringStyle.strokeWidth, ringStyle.stroke, ringStyle.strokeAlpha);
    indicator.setVisible(true);
