...
from agora_ui.live_world import load_pixel_world_context, shutdown_registered_live_stores


ROOT = Path(__file__).resolve().parents[1]
HEADLESS_PIXEL_SCRIPT = ROOT / "scripts" / "headless_pixel_firefox_regression.py"
_TEST_REFERENCE_ACCESS_CODE = f"tst{os.getpid():013d}"
_TEST_REFERENCE_EXPORT_CREATED = False


def _remove_test_reference_export() -> None:
    if _TEST_REFERENCE_EXPORT_CREATED:
        shutil.rmtree(
            ROOT / "output" / "package_exports" / _TEST_REFERENCE_ACCESS_CODE,
            ignore_errors=True,
        )


atexit.register(_remove_test_reference_export)


def _create_test_reference_export() -> str:
    global _TEST_REFERENCE_EXPORT_CREATED

    source_db = ROOT / "sample_json" / "world_package.db"
    if not source_db.is_file():
        raise AssertionError(f"Missing reference package: {source_db}")
    export_dir = ROOT / "output" / "package_exports" / _TEST_REFERENCE_ACCESS_CODE
    if export_dir.exists():
        raise AssertionError(f"Refusing to replace existing test export: {export_dir}")
    export_dir.mkdir(parents=True)
    target_db = export_dir / "world_package.db"
    ...
    pixel_report = {
        ...
        "missing_resources": [],
        "details": ["Self-contained reference package for clean-install tests."],
    }
    metadata = {
        "access_code": _TEST_REFERENCE_ACCESS_CODE,
        "created_at": "2026-07-30T00:00:00Z",
        "package_name": "agora_reference_test_world",
        "source_label": "macro_ui_export",
        "pixel_read": True,
    ...
    with sqlite3.connect(target_db) as conn:
        ...
        conn.commit()
    (export_dir / "package_meta.json").write_text(
        json.dumps(metadata, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    _TEST_REFERENCE_EXPORT_CREATED = True
    return _TEST_REFERENCE_ACCESS_CODE


class _StreamingReplyClient:
    def stream_generate_compact_json_field(self, **_: dict):
        payload = self.generate_json()
        yield str(payload.get("response_text", "")), True, payload


class _LiveReplyClient(_StreamingReplyClient):
...
class _LiveToolReplyClient(_StreamingReplyClient):
    def __init__(self, config: dict) -> None:
        self.config = config
...
class _LiveRouteTradeReplyClient(_StreamingReplyClient):
    def __init__(self, config: dict) -> None:
        self.config = config
...
class _LiveEmptyThenValidReplyClient(_StreamingReplyClient):
    attempts = 0
...
def _first_pixel_world_access_code() -> str:
    for record in serve_macro_ui._latest_pixel_world_records():
        access_code = str(record.get("access_code", "")).strip()
        if access_code and not bool(record.get("validation_probe")):
            return access_code
    access_code = _create_test_reference_export()
    if serve_macro_ui._pixel_world_record(access_code) is None:
        raise AssertionError("Self-contained PIXEL READ reference package is invalid")
    return access_code


@lru_cache(maxsize=8)
def _shared_agent_room_id(access_code: str) -> str:
    context = load_pixel_world_context(str(ROOT), access_code)
    room_counts: dict[str, int] = {}
    for agent in context.agent_seed_payloads:
        room_id = str(agent.get("room_id", "")).strip()
        if room_id:
            room_counts[room_id] = room_counts.get(room_id, 0) + 1
    candidates = sorted(room_id for room_id, count in room_counts.items() if count >= 2)
    if not candidates:
        raise AssertionError(f"No room with at least two agents in Pixel world {access_code}")
    return candidates[0]
...
class PixelLiveWorldApiTest(unittest.TestCase):
    ...
    def test_live_session_flow_claims_agent_and_persists_actions(self) -> None:
        ...
        with self._live_temp_root(access_code) as temp_root:
            with (
                ...
                patch("agora_ui.live_world.mixin_core_ai._load_vertex_json_client_class", return_value=_LiveReplyClient),
            ):
                create_payload = serve_macro_ui.api_create_live_session(
                    ...
                    serve_macro_ui.PixelLiveSessionCreateRequest(
                        ...
                        room_id=_shared_agent_room_id(access_code),
    ...
    def test_headless_pixel_harness_targets_requested_world_and_resets_client_state(self) -> None:
        ...
        self.assertIn("const requireAiReply = true;", html)
        self.assertIn("ai_reply_checked: requireAiReply", html)

    def test_headless_pixel_harness_has_explicit_deterministic_publish_profile(self) -> None:
        html = serve_macro_ui._render_headless_pixel_harness(
            42627,
            "token1234",
            "83a27fc6b177b058",
            require_ai_reply=False,
        )
        self.assertIn("const requireAiReply = false;", html)
        self.assertIn("if (requireAiReply)", html)
        self.assertIn("agent_reply_ms: requireAiReply", html)
        ...
        with self._live_temp_root(access_code) as temp_root:
            with (
                ...
                message_state = self._wait_for_live_condition(
                    ...
                    session_id=session["session_id"],
                    predicate=lambda payload: any(
                        event["event_type"] == "agent_response" and event["action_text"] == user_text
                        for event in payload["events"]
                    ),
                )
                ...
                response_events = [
                    event
                    for event in message_state["events"]
                    if event["event_type"] == "agent_response" and event["action_text"] == user_text
                ]
    ...
    def test_live_message_tool_call_assigns_follow_task(self) -> None:
        ...
        with self._live_temp_root(access_code) as temp_root:
            with (
                patch.object(serve_macro_ui, "MACRO_PACKAGE_ROOT", temp_root),
                patch.dict("os.environ", {"AGORA_AISTUDIO_API_KEY": "test-key"}, clear=False),
                patch("agora_ui.live_world.mixin_core_ai._load_vertex_json_client_class", return_value=_LiveToolReplyClient),
            ):
                create_payload = serve_macro_ui.api_create_live_session(
                    access_code,
                    serve_macro_ui.PixelLiveSessionCreateRequest(
                ...
                message_state = self._wait_for_live_condition(
                    ...
                    session_id=session["session_id"],
                    predicate=lambda payload: any(
                        agent["agent_id"] == target_agent["agent_id"]
                        and isinstance(agent.get("active_task"), dict)
                ...
                response_events = [
                    ...
                    and event["action_text"] == "Come with me and stay close."
                ]
                self.assertTrue(response_events)
    ...
    def test_live_route_catalog_is_seeded_and_ai_route_selection_quotes_trade(self) -> None:
        ...
        with self._live_temp_root(access_code) as temp_root:
            with (
                patch.object(serve_macro_ui, "MACRO_PACKAGE_ROOT", temp_root),
                patch.dict("os.environ", {"AGORA_AISTUDIO_API_KEY": "test-key"}, clear=False),
                patch("agora_ui.live_world.mixin_core_ai._load_vertex_json_client_class", return_value=_LiveRouteTradeReplyClient),
            ):
                create_payload = serve_macro_ui.api_create_live_session(
                    access_code,
                    serve_macro_ui.PixelLiveSessionCreateRequest(
                ...
                message_state = self._wait_for_live_condition(
                    ...
                    predicate=lambda payload: (
                        ...
                        and any(
                            ...
                            and event["action_text"] == "Can you sell me that crystal?"
                            for event in payload["events"]
                        )
                    ),
                )
    ...
    def test_live_message_retries_when_ai_returns_empty_response(self) -> None:
        ...
        with self._live_temp_root(access_code) as temp_root:
            ...
            with (
                patch.object(serve_macro_ui, "MACRO_PACKAGE_ROOT", temp_root),
                patch.dict("os.environ", {"AGORA_AISTUDIO_API_KEY": "test-key"}, clear=False),
                patch("agora_ui.live_world.mixin_core_ai._load_vertex_json_client_class", return_value=_LiveEmptyThenValidReplyClient),
            ):
                create_payload = serve_macro_ui.api_create_live_session(
                    access_code,
                    serve_macro_ui.PixelLiveSessionCreateRequest(
                ...
                message_state = self._wait_for_live_condition(
                    ...
                    session_id=session["session_id"],
                    predicate=lambda payload: (
                        _LiveEmptyThenValidReplyClient.attempts >= 2
                        and any(
                            event["event_type"] == "agent_response"
                            and event["action_text"] == "Retry the empty answer"
                            for event in payload["events"]
                        )
                    ),
                )
                ...
                last_event = next(
                    event
                    for event in reversed(message_state["events"])
                    if event["event_type"] == "agent_response"
                    and event["action_text"] == "Retry the empty answer"
                )
        ...
        with self._live_temp_root(access_code) as temp_root:
            with (
                ...
                if not agents_dir.is_dir():
                    agents_dir = workspace / "scenario" / "Agents"
