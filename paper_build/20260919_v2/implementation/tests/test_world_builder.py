...
def _strictify_builder_spec(spec: dict[str, Any]) -> dict[str, Any]:
    policy_refs = dict(spec.get("world_seed", {}).get("policy_refs", {}))
    collection_id = str(policy_refs.get("item_collection_id", ""))
    catalog = [
        dict(item)
        for item in world_builder.ITEM_COLLECTION_REGISTRY[collection_id]["item_catalog"]
        if isinstance(item, dict)
    ]
    usable_items = [
        item
        for item in catalog
        if not bool(dict(item.get("metadata", {})).get("forbidden_for_generated_inventory", False))
    ]
    usable_ids = [str(item.get("item_id", "")) for item in usable_items if str(item.get("item_id", "")).strip()]
    for index, role in enumerate(spec.get("role_groups", [])):
        role["starting_item_ids"] = usable_ids[index:index + 4] or usable_ids[:4]
        role["property_templates"] = [
            {
                "asset_name": f"{role.get('role_name', 'Role')} Worksite {offset + 1}",
                "asset_type": "worksite",
                "description": "A world-specific operational asset.",
                "story_use": "Creates visible obligations and access pressure.",
            }
            for offset in range(2)
        ]
        role["knowledge_templates"] = [
            {
                "knowledge_id": f"role_{index}_knowledge_{offset}",
                "topic": "local operations",
                "summary": "Actionable knowledge tied to this role and world.",
                "confidence": 70 + offset,
            }
            for offset in range(2)
        ]
    inventory_source = usable_items[:15] or catalog[:15]
    character_names = [
        str(character.get("display_name", f"Character {index + 1}"))
        for index, character in enumerate(spec.get("main_characters", []))
    ]
    for index, character in enumerate(spec.get("main_characters", [])):
        character["inventory"] = [
            {
                "name": str(item.get("name", item.get("item_id", f"Item {offset + 1}"))),
                "description": str(item.get("description", "A world-specific carried item.")) or "A world-specific carried item.",
                "quantity": 1 + (offset % 2),
            }
            for offset, item in enumerate(inventory_source)
        ]
        character["property_templates"] = [
            {
                "asset_name": f"{character.get('display_name', 'Character')} Asset {offset + 1}",
                "asset_type": "personal",
                "description": "A concrete world-specific property.",
                "story_use": "Supports trade, access, and conflict.",
            }
            for offset in range(2)
        ]
        character["knowledge_templates"] = [
            {
                "knowledge_id": f"character_{index}_knowledge_{offset}",
                "topic": "local leverage",
                "summary": "Private, actionable knowledge with persistent consequences.",
                "confidence": 75 + offset,
            }
            for offset in range(2)
        ]
        if len(character_names) > 1:
            character["relationships"] = [
                {
                    "target_name": character_names[(index + offset + 1) % len(character_names)],
                    "relationship_type": ["supplier", "rival"][offset],
                    "description": "A concrete recurring dependency in the world simulation.",
                }
                for offset in range(min(2, len(character_names) - 1))
            ]
    spec["player_entry_points"] = [
        "Arrive during a live dispute and identify the first concrete obligation.",
        "Follow a contested item or route to the agent currently blocking it.",
        "Use conversation, inspection, and trade to make one persistent change.",
    ]
    spec["agent_count_target"] = len(spec.get("main_characters", []))
    return spec


def _compiler_repair_critique() -> dict[str, Any]:
    return {
        "should_repair": True,
        "diagnosis": ["Player onboarding and customs pressure need one more concrete loop."],
        "custom_actions": ["Debate"],
        "player_entry_points": ["Arrive during a customs dispute and learn the harbor through competing explanations."],
        "conflict_hooks": ["A customs inspection wave makes ordinary trade politically risky."],
        "social_rules": ["Inspection pressure must force route and information choices into the open."],
        "loop_reinforcements": [{
            "label": "Inspection Loop",
            "summary": "Agents reroute cargo, negotiate permits, and trade information during customs checks.",
            "roles": [],
            "rooms": ["Harbor District 1", "Harbor District 2"],
...
class _CreatorProvider:
    def __init__(self) -> None:
        ...
        self.generated_rooms: list[dict[str, Any]] = []
        ...
        if request.task_type == "json":
            ...
            return FlexExecuteResponse(
                ...
                output_json={
                    ...
                    "world_dynamics": {
                        "institutions": [
                            {"institution_id": "tide_board", "name": "Tide Board", "mandate": "allocate safe passage", "decision_rule": "rank verified need and public obligation", "enforced_by": "signal pilots", "visible_output": "lit berth signals"},
                            {"institution_id": "cargo_compact", "name": "Cargo Compact", "mandate": "settle shared deliveries", "decision_rule": "weighted member vote", "enforced_by": "dock brokers", "visible_output": "sealed cargo schedule"},
                            {"institution_id": "shrine_archive", "name": "Shrine Archive", "mandate": "certify tidal evidence", "decision_rule": "two independent readings", "enforced_by": "archive keepers", "visible_output": "public omen record"},
                        ],
                        "state_variables": [
                            {"state_id": "route_access", "label": "Route access", "initial_tension": "contested", "changed_by": ["signals"], "affects": ["supply"]},
                            {"state_id": "supply", "label": "Civic supply", "initial_tension": "thin", "changed_by": ["deliveries"], "affects": ["trust"]},
                            {"state_id": "trust", "label": "Public trust", "initial_tension": "fragmented", "changed_by": ["kept promises"], "affects": ["votes"]},
                            {"state_id": "signal_truth", "label": "Signal truth", "initial_tension": "uncertain", "changed_by": ["evidence"], "affects": ["route_access"]},
                        ],
                        "causal_links": [
                            {"source_state": "signal_truth", "trigger": "archive verifies a reading", "target_state": "route_access", "effect": "a channel opens", "delay": "next tide"},
                            {"source_state": "route_access", "trigger": "a berth is allocated", "target_state": "supply", "effect": "goods reach a district", "delay": "one round"},
                            {"source_state": "supply", "trigger": "a promise is fulfilled", "target_state": "trust", "effect": "priority shifts", "delay": "next vote"},
                            {"source_state": "trust", "trigger": "a coalition forms", "target_state": "signal_truth", "effect": "new evidence is funded", "delay": "two rounds"},
                        ],
                        "stakeholder_groups": [
                            {"group_id": "brokers", "name": "Dock Brokers", "goal": "reliable margins", "leverage": "cargo contracts", "dependency": "pilot access"},
                            {"group_id": "pilots", "name": "Tide Pilots", "goal": "safe routes", "leverage": "signal knowledge", "dependency": "archive evidence"},
                            {"group_id": "keepers", "name": "Shrine Keepers", "goal": "credible records", "leverage": "certification", "dependency": "public trust"},
                        ],
                    },
    ...
    def generate_json(self, *, system_instruction: str, prompt: str, schema: dict[str, Any], stage: str = "") -> dict[str, Any] | list[Any]:
        ...
        if stage == "world_visual_canon_generation" or "canon_id" in required:
            return {
                "canon_id": "clockharbor_visual_canon",
                "camera": {
                    "projection": "strict_top_down",
                    "pitch": "orthographic overhead",
                    "scale_rule": "one tile equals one stable gameplay unit",
                },
                "palette": {
                    "dominant": "#34505f",
                    "secondary": "#8a6f4d",
                ...
                "materials": ["weathered brass", "salt-dark timber", "blue-gray stone"],
                "lighting": "cool harbor daylight with warm signal lamps",
                "architecture": ["low dock sheds", "brass signal towers", "stone tidal channels"],
                "terrain": {
                    "ground": "salt-worn quay stone",
                    "path": "brass-edged cargo lanes",
                    "edge": "dark tidal water and retaining walls",
                },
                "sprite_language": {
                    "body_proportion": "readable natural JRPG proportions",
                    "outline": "dark single-pixel outline",
                    "role_readability": "silhouette and gear reveal occupation",
                },
                "world_prompt_prefix": "Strict top-down harbor city pixel art with weathered brass and tidal infrastructure",
                "sprite_prompt_prefix": "Natural-proportion harbor worker sprite with role-specific equipment",
                "forbidden_visuals": ["visible text", "poster framing", "blank margins", "Beijing antique clothing"],
            }
        if stage == "agent_wardrobe_policy_generation" or "policy_id" in required:
            rule = lambda index: {
                "rule_id": f"harbor_role_{index}",
                "role_keywords": [f"role{index}", "pilot" if index == 1 else "specialist"],
                "attire": [f"layered harbor utility coat {index}", f"weatherproof duty rig {index}"],
                "props": [f"signal tool {index}", f"route instrument {index}"],
                "palette_notes": "Keep the brass accent subordinate to the blue-gray outerwear.",
                "silhouette_notes": "Readable natural full-body silhouette with role gear.",
                "avoid": ["generic fantasy robes"],
            }
            return {
                "policy_id": "clockharbor_wardrobe",
                "world_wardrobe_brief": "Weatherproof harbor workwear tied to trade, signals, and tidal logistics.",
                "forbidden_aesthetics": ["antique-market vendor clothing", "generic cyberpunk neon", "medieval robes"],
                "palette_families": [
                    {"name": "quay_blue", "dominant": "blue gray", "accent": "signal yellow", "trim": "brass", "material": "waxed canvas"},
                    {"name": "tide_green", "dominant": "deep green", "accent": "ivory", "trim": "copper", "material": "oilcloth"},
                    {"name": "storm_red", "dominant": "oxblood", "accent": "cold blue", "trim": "steel", "material": "heavy wool"},
                ],
                "role_style_rules": [rule(1), rule(2), rule(3)],
                "default_rule": rule(0),
                "visual_consistency_rules": ["Preserve natural proportions.", "Keep role gear visible.", "Use the shared harbor palette."],
            }
        if "characters" in required:
            match = re.search(r"Generate exactly\s+(\d+)\s+highly detailed", prompt)
            expected = int(match.group(1)) if match else 1
            characters = []
            room_names = [str(room.get("name", "")) for room in self.generated_rooms] or ["Tide Market"]
            batch_names = [
                f"Clockharbor Citizen {self.character_index + offset + 1}"
                for offset in range(expected)
            ]
            available_names = [
                f"Clockharbor Citizen {index + 1}"
                for index in range(self.character_index + expected)
            ]
            for offset in range(expected):
                index = self.character_index
                self.character_index += 1
                display_name = f"Clockharbor Citizen {index + 1}"
                relationship_targets = list(
                    dict.fromkeys(
                        name for name in [*batch_names, *available_names]
                        if name != display_name
                    )
                )[:2]
                characters.append({
                    "display_name": display_name,
                    "role_name": "Tide Specialist",
                    "activity": "Coordinates a specific harbor route and negotiates visible obligations.",
                    "home_base": room_names[index % len(room_names)],
                    "arc_goal": "Turn a contested route into durable civic leverage.",
                    "inventory": [
                        {
                            "name": f"Harbor Tool {index + 1}-{item_index + 1}",
                            "description": "A concrete Clockharbor instrument used in trade and route work.",
                    ...
                    "property_templates": [
                        {
                            "asset_name": f"Berth Claim {index + 1}-{asset_index + 1}",
                            "asset_type": "harbor_claim",
                            "description": "A persistent claim on working harbor infrastructure.",
                            "story_use": "Creates access pressure and negotiable obligations.",
                        }
                        for asset_index in range(2)
                    ],
                    "knowledge_templates": [
                        {
                            "knowledge_id": f"route_secret_{index + 1}_{knowledge_index + 1}",
                            "topic": "route conditions",
                            "summary": "Actionable knowledge about a contested tidal route.",
                            "confidence": 78 + knowledge_index,
                        }
                        for knowledge_index in range(2)
                    ],
                    "relationships": [
                        {
                            "target_name": target_name,
                            "relationship_type": ["supplier", "rival"][relation_index],
                            "description": "A recurring harbor dependency with visible obligations.",
                        }
                        for relation_index, target_name in enumerate(relationship_targets)
                    ],
                })
            return {"characters": characters}
        if "rooms" in required:
            match = re.search(r"(?:AT LEAST|EXACTLY)\s+(\d+)", prompt)
            room_count = int(match.group(1)) if match else 6
            self.generated_rooms = [
                {
                    "name": f"Harbor District {index + 1}",
                    "biome": "tidal trade infrastructure",
                    "purpose": f"Handles route function {index + 1} and public negotiation.",
                    "decor_tags": ["brass_signals", "cargo_lines"],
                    "activity_tags": ["route", "trade"],
                    "archetype": ["market_exchange", "lookout", "logistics_yard"][index % 3],
                    "ambient_palette": ["clear_day", "focused_blue", "warm_lantern"][index % 3],
                    "width_tiles": 4 + (index % 2),
                    "height_tiles": 4 + (index % 3),
                    "flux_floor_prompt": "Top-down pixel art salt-worn quay floor texture, no text, edge-to-edge.",
                    "room_scene_prompt": "Strict top-down harbor room plate with brass route equipment, no labels or blank margins.",
                    "scene_components": [{
                        "component_id": f"district_{index + 1}_tide_instrument",
                        "label": f"Tide Instrument {index + 1}",
                        "description": "A large world-specific brass instrument that makes this district's route function legible.",
                        "anchor": "north_east",
                        "width_tiles": 2,
                        "height_tiles": 2,
                    }],
            ...
            return {"wall_color_theme": "weathered_brass", "outdoor_terrain": "concrete", "rooms": self.generated_rooms}
        if "room_materials" in required:
            return {
                "room_materials": [
                    {
                        "room_name": str(room.get("name", "")),
                        "floor_tile": "stone_slabs",
                        "wall_tile": "storage_wall",
                        "ambient_palette": "clear_day",
                        "showcase_shelf": False,
                        "showcase_item_colors": [],
                        "reflection_glares": False,
                    }
                    for room in self.generated_rooms
                ]
            }
        if "item_catalog" in required:
            match = re.search(r"AT LEAST\s+(\d+)", prompt)
            item_count = int(match.group(1)) if match else 18
            return {
                "item_catalog": [
                    {
                        "item_id": f"harbor_item_{index + 1:02d}",
                        "name": f"Harbor Item {index + 1}",
                        "price": 20 + index,
                        "mass": 0.2 + (index % 4) * 0.1,
                        "description": "A world-specific instrument used in Clockharbor trade.",
                        "image_prompt": "Top-down pixel-art harbor trade instrument icon, isolated, no text.",
                        "category": "harbor_trade",
                    }
                    for index in range(item_count)
                ]
            }
        if "gameplay_loops" in required:
            loop_properties = properties["gameplay_loops"]["items"]["properties"]
            room_names = loop_properties["rooms"]["items"].get("enum", ["Tide Market"])
            role_names = loop_properties["roles"]["items"].get("enum", ["Harbor Broker"])
            return {
                "social_rules": ["Every route promise creates a visible obligation.", "Signal knowledge can be traded but leaves a trace."],
                "custom_actions": ["BidForTideWindow", "SealCargoCompact", "ContestSignalReading", "FundArchiveProof", "RedirectReliefBarge", "CallBerthVote"],
                "open_action_examples": [
                    {"action_name": f"Harbor Proposal {index + 1}", "initiating_role": role_names[index % len(role_names)], "target_role": role_names[(index + 1) % len(role_names)], "situation": "A contested tide window creates an unplanned bargain.", "proposed_effects": [["speech"], ["transfer"], ["create_item"], ["status"]][index], "coordinator_checks": ["same room", "ownership or consent", "bounded effect"]}
                    for index in range(4)
                ],
                "player_entry_points": ["Arrive during a contested tide window."],
                "conflict_hooks": ["Two pilots claim the same safe channel.", "A missing signal ledger changes cargo priority."],
                "gameplay_loops": [
                    {"label": "Route Loop", "summary": "Trade route knowledge for access.", "roles": [role_names[0]], "rooms": [room_names[0]], "pressure": "closing tide"},
                    {"label": "Obligation Loop", "summary": "Convert promises into visible deliveries.", "roles": [role_names[-1]], "rooms": [room_names[-1]], "pressure": "unpaid claims"},
                    {"label": "Evidence Loop", "summary": "Verify a signal and change the next route allocation.", "roles": [role_names[0]], "rooms": [room_names[-1]], "pressure": "unverified readings"},
                ],
            }
        if "should_repair" in required:
            return {
                "should_repair": False,
                "diagnosis": [],
                "custom_actions": [],
...
class _CritiquingCreatorProvider(_CreatorProvider):
    ...
    def generate_json(self, *, system_instruction: str, prompt: str, schema: dict[str, Any], stage: str = "") -> dict[str, Any] | list[Any]:
        if "should_repair" in set(schema.get("required", [])):
            class DummyRequest:
                task_type = "json"
            self.json_calls = 1
            return self.execute(DummyRequest()).output_json
...
class WorldBuilderApiTest(unittest.TestCase):
    def setUp(self) -> None:
        self._inline_generation_env = patch.dict(os.environ, {"AGORA_WORLD_CREATOR_INLINE": "1"})
        self._inline_generation_env.start()
        self._map_qa = patch(
            "agora_ui.world_builder.art._run_gemini_map_qa",
            return_value={
                "status": "ok",
                "reasoning": "Synthetic map QA pass for the isolated unit test.",
                "is_pixel_map": "Y",
                "has_visual_errors": "N",
                "error_codes": [],
            },
    ...
    def tearDown(self) -> None:
        self._map_qa.stop()
        self._inline_generation_env.stop()
    ...
    def test_compiler_strengthens_loops_routes_and_role_directives(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            ...
            builder_spec = _strictify_builder_spec(builder_spec)
            ...
            self.assertTrue(config["actions"]["ordinary_routes"])
            self.assertIn("CinematicInteraction", config["actions"]["allowed_custom_actions"])
            self.assertTrue(config["world_rules"]["custom_action_rules"]["open_proposals_enabled"])
            self.assertEqual(config["actions"]["routing_policy"]["open_proposal_route_id"], "__propose__")
            ...
            self.assertEqual(config["agent_generation"]["role_groups"], [])
            self.assertTrue(all(str(character.get("home_room_id", "")).strip() for character in config["main_characters"]))
            self.assertTrue(all(str(character.get("activity_directive", "")).strip() for character in config["main_characters"]))
            main_character_ids = {
                str(character.get("agent_id", "")).strip()
                for character in config["main_characters"]
            }
            self.assertTrue(
                all(len(character.get("relationships", [])) >= 2 for character in config["main_characters"])
            )
            self.assertTrue(
                all(
                    str(relationship.get("target_agent_id", "")).strip() in main_character_ids
                    and str(relationship.get("target_agent_id", "")).strip()
                    != str(character.get("agent_id", "")).strip()
                    for character in config["main_characters"]
            ...
            materialized_rules = json.loads((scenario_dir / "world_rules.json").read_text(encoding="utf-8"))
            self.assertTrue(materialized_rules["custom_action_rules"]["open_proposals_enabled"])
    ...
    def test_compiler_emits_world_neutral_ids_and_market_inventory(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            ...
            protagonist_items = [
                ...
                for item in config["main_characters"][0].get("inventory", [])
                if isinstance(item, dict)
            ]
            self.assertGreaterEqual(len(protagonist_items), 8)
            self.assertFalse({"task_ledger", "meeting_notice", "tea_coupon"} & set(protagonist_items))
            self.assertNotIn("gold", protagonist_items)
    ...
    def test_revise_failed_draft_reuses_original_input_brief(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            ...
            with (
                patch.object(serve_macro_ui, "MACRO_PACKAGE_ROOT", temp_root),
                patch("agora_ui.world_builder.core._generate_revision", side_effect=_fake_generate_revision),
            ):
                revised = serve_macro_ui.api_world_builder_revise_draft(
                    draft["draft_id"],
                    serve_macro_ui.WorldBuilderDraftReviseRequest(
    ...
    def test_compiler_critique_auto_repairs_world_config(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            ...
            with (
                ...
                patch.object(world_builder, "_world_creator_provider", return_value=provider),
                patch(
                    "agora_ui.world_builder.generation._critique_compiled_world_config",
                    return_value=_compiler_repair_critique(),
                ),
            ):
                draft = serve_macro_ui.api_world_builder_create_draft(
                    serve_macro_ui.WorldBuilderDraftCreateRequest(
    ...
    def test_art_pipeline_and_publish_make_world_public(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            ...
            with (
                ...
                self.assertEqual(art_payload["status"], world_builder.STATUS_PUBLISH_READY, art_payload)
