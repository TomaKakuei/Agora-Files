...
from agora_ui.world_pipeline import ITEM_COLLECTION_REGISTRY
...
def _strictify_builder_spec(spec: dict[str, Any]) -> dict[str, Any]:
    ...
    catalog = [
        dict(item)
        for item in ITEM_COLLECTION_REGISTRY[collection_id]["item_catalog"]
    ...
    usable_items = [
        ...
        and not bool(dict(item.get("metadata", {})).get("currency", False))
        and not bool(
            dict(item.get("metadata", {})).get(
                "forbidden_for_generated_inventory", False
            )
        )
    ]
    usable_ids = [str(item["item_id"]) for item in usable_items]
    for role_index, role in enumerate(spec.get("role_groups", [])):
        role["starting_item_ids"] = usable_ids[role_index : role_index + 4] or usable_ids[:4]
        role["property_templates"] = [
            {
                "asset_name": f"{role.get('role_name', 'Role')} Worksite {offset + 1}",
                "asset_type": "worksite",
                "description": "A concrete operational asset specific to this world.",
                "story_use": "Creates visible obligations, access, and trade pressure.",
            }
            for offset in range(2)
        ]
        role["knowledge_templates"] = [
            {
                "knowledge_id": f"role_{role_index}_knowledge_{offset}",
    ...
    for character_index, character in enumerate(spec.get("main_characters", [])):
        character["inventory"] = [
            {
                "item_id": str(item["item_id"]),
                "name": str(item.get("name", item["item_id"])),
                "description": str(item.get("description", "")).strip()
                or "A concrete world-specific carried item.",
                "quantity": 1 + (item_index % 2),
            }
            for item_index, item in enumerate(usable_items[:12])
        ]
        ...
        character["knowledge_templates"] = [
            {
                "knowledge_id": f"character_{character_index}_knowledge_{offset}",
...
def _enrich_config_main_character_inventories(config: dict[str, Any]) -> None:
    catalog = [
        dict(item)
        for item in config.get("property_library", {}).get("item_catalog", [])
        if isinstance(item, dict)
    ]
    usable_items = [
        item
    ...
    for character_index, character in enumerate(config.get("main_characters", [])):
        character["inventory"] = [
            {"item_id": str(item["item_id"]), "quantity": 1 + (index % 2)}
            for index, item in enumerate(usable_items[:8])
        ]
        character["property_library"] = [
            {
                "asset_id": f"migration_property_{character_index}_{index}",
                "name": f"Registered property {index + 1}",
                "description": "Explicit test fixture property for registered-world packaging.",
            }
            for index in range(2)
        ]
        character["knowledge_assets"] = [
            {
                "knowledge_id": f"migration_knowledge_{character_index}_{index}",
                "topic": "registered world",
                "summary": "Explicit test fixture knowledge for registered-world packaging.",
                "confidence": 75 + index,
            }
            for index in range(2)
        ]
...
class WorldDefinitionTest(unittest.TestCase):
    ...
    def test_pack_world_package_writes_structured_world_definition_tables(self) -> None:
        ...
        config["scenario_meta"]["description"] = "A realistic antique market full of appraisal disputes."
        config = sync_world_definition_into_config(config)
        _enrich_config_main_character_inventories(config)
    ...
    def test_world_builder_panjiayuan_avoids_legacy_guild_outputs(self) -> None:
        ...
        generated_world_payload = {
            "main_characters": config["main_characters"],
            "item_catalog": config["property_library"]["item_catalog"],
            "world_definition": config["world_definition"],
        }
        dumped = json.dumps(generated_world_payload, ensure_ascii=False)
