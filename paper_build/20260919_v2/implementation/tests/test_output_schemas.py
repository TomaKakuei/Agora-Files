...
def _sample_bootstrap_agents() -> dict:
    path = _first_existing([
        'frontend/bootstrap_agents.json',
        'frontend/assets/generated/bootstrap_agents.json',
    ])
    if path is not None:
...
def _sample_asset_event() -> dict:
    path = _first_existing(['frontend/assets/generated/events/latest.json'])
    if path is not None:
        try:
...
class OutputSchemasTest(unittest.TestCase):
    def test_bootstrap_agents_sample(self) -> None:
        payload = _sample_bootstrap_agents()
