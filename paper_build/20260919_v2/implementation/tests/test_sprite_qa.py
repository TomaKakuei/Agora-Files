...
class SpriteQATest(unittest.TestCase):
    def test_build_prompt_bundle_injects_anti_crop_guardrails(self) -> None:
        ...
        self.assertIn("normal full-body human silhouette", bundle["sprite_prompt"])
        self.assertIn("Do not use super-deformed chibi sticker proportions", bundle["sprite_prompt"])
        self.assertIn("generous transparent margins", bundle["sprite_prompt"])
        self.assertIn("centered in invisible grid cells", bundle["sprite_prompt"])
    ...
    def test_strict_programmatic_qa_accepts_removable_near_white_background(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            ...
            self.assertTrue(result["pass_qa"])
            self.assertTrue(result["checks"]["background_transparency"]["pass"])
            self.assertEqual(result["checks"]["background_transparency"]["source_transparent_ratio"], 0.0)
            self.assertGreater(result["checks"]["background_transparency"]["processed_transparent_ratio"], 0.2)

    def test_strict_programmatic_qa_rejects_non_removable_opaque_background(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            image_path = Path(tmp_dir) / "sheet_opaque_colored.png"
            states = _default_states()
            _make_sheet(
                image_path,
                columns=4,
            ...
            background = Image.new("RGBA", sprite.size, (24, 38, 52, 255))
    ...
    def test_atlas_quality_uses_configured_64px_frame_size(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            ...
            atlas = build_phaser_atlas(
                ...
                neutral_tolerance=12,
                animation_states=states,
                alignment_policy={"anchor_bottom_px": 60},
            )

            quality = atlas["meta"]["quality_summary"]
            state_reports = atlas["meta"]["quality_report"]["states"]
            self.assertTrue(quality["pass"])
            self.assertTrue(all(report["max_body_height_ratio"] <= 1.0 for report in state_reports))
    ...
    def test_final_atlas_transparency_qa_rejects_white_background(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            ...
            self.assertGreater(
                result["frame_edge_connected_near_white_ratio"],
                0.99,
            )

    def test_final_atlas_transparency_qa_allows_interior_white_clothing(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            atlas_path = Path(tmp_dir) / "white_clothing_atlas.png"
            atlas = Image.new("RGBA", (32, 32), (0, 0, 0, 0))
            draw = ImageDraw.Draw(atlas)
            draw.rectangle((10, 4, 21, 28), fill=(252, 252, 252, 255))
            atlas.save(atlas_path)

            result = final_atlas_transparency_qa(image_path=str(atlas_path))

            self.assertTrue(result["pass"])
            self.assertGreater(result["near_white_opaque_ratio"], 0.01)
            self.assertEqual(result["frame_edge_connected_near_white_ratio"], 0.0)
    ...
    @patch("asset_pipeline.sprite_qa_vision._load_runtime_clients", return_value=_VisionSuccessClient)
    ...
    @patch("asset_pipeline.sprite_qa_vision._load_runtime_clients", return_value=_VisionBackgroundFailClient)
    ...
    @patch("asset_pipeline.sprite_qa_vision._load_runtime_clients", return_value=_VisionTimeoutClient)
    ...
    def test_generate_agent_assets_rejects_incomplete_raw_sheet_even_when_legacy_fallback_is_enabled(self, mock_run_combined_qa: object) -> None:
        ...
        with TemporaryDirectory(dir=frontend_root) as tmp_dir:
            ...
            self.assertFalse(raw_sheet_report["pass_qa"])
            self.assertFalse(bundle_path.exists())
            self.assertFalse((event_root / "latest.json").exists())

    @patch("asset_pipeline.generate_agent_assets.run_combined_qa")
    def test_generate_agent_assets_rejects_incomplete_raw_when_procedural_fallback_disabled(self, mock_run_combined_qa: object) -> None:
        mock_run_combined_qa.return_value = {
        ...
        with TemporaryDirectory(dir=frontend_root) as tmp_dir:
            ...
            with patch.object(sys, "argv", argv):
                with self.assertRaises(SystemExit):
                    generate_agent_assets_main()
            self.assertFalse((output_root / "guild_main_princess" / "qa_no_backfill" / "asset_bundle.json").exists())
            self.assertFalse((event_root / "latest.json").exists())

    @patch("asset_pipeline.generate_agent_assets.run_combined_qa")
    def test_generate_agent_assets_rejects_deprecated_procedural_bootstrap(self, mock_run_combined_qa: object) -> None:
        mock_run_combined_qa.return_value = {
            "programmatic_qa": {"pass_qa": True},
            "vision_qa": {"status": "skipped", "pass_qa": None},
            "overall_status": "pass",
        ...
        with TemporaryDirectory(dir=frontend_root) as tmp_dir:
            ...
            with patch.object(sys, "argv", argv):
                with self.assertRaisesRegex(RuntimeError, "Procedural sprite bootstrap is disabled"):
                    generate_agent_assets_main()
            mock_run_combined_qa.assert_not_called()
