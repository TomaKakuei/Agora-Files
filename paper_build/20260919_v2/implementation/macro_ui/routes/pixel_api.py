...
_LATEST_WORLD_CACHE: dict[str, object] = {
    "root": "",
    "root_mtime_ns": -1,
    "expires_at": 0.0,
    "records": [],
...
def _latest_pixel_world_records() -> list[dict[str, object]]:
    export_root = serve_macro_ui._package_export_root()
    try:
        root_mtime_ns = export_root.stat().st_mtime_ns
    except OSError:
        root_mtime_ns = -1
    now = time.monotonic()
    if (
        str(_LATEST_WORLD_CACHE.get("root", "")) == str(export_root)
        and int(_LATEST_WORLD_CACHE.get("root_mtime_ns", -2)) == root_mtime_ns
        and float(_LATEST_WORLD_CACHE.get("expires_at", 0.0)) > now
    ):
        cached = _LATEST_WORLD_CACHE.get("records", [])
        return [dict(record) for record in cached if isinstance(record, dict)]
...
def _require_pixel_live_access_code(access_code: str) -> str:
    normalized = str(access_code).strip()
    if len(normalized) != 16:
        raise HTTPException(status_code=400, detail="access code must be 16 characters")
    if not (_package_export_dir(normalized) / "world_package.db").is_file():
        raise HTTPException(status_code=404, detail=f"Pixel world package not found: {normalized}")
    return normalized
...
def _pixel_world_record(access_code: str) -> dict[str, object] | None:
    ...
    try:
        ...
        metadata_path = export_dir / "package_meta.json"
        file_meta: dict[str, Any] = {}
        if metadata_path.is_file():
            loaded_meta = json.loads(metadata_path.read_text(encoding="utf-8"))
            if isinstance(loaded_meta, dict):
                file_meta = loaded_meta
        metadata = {**file_meta, **package_meta}
        pixel_read = _metadata_bool(package_meta, "pixel_read")
        if pixel_read is None:
            pixel_read = _metadata_bool(metadata, "pixel_read")
        if pixel_read is False:
            return None
        startup_ok = _metadata_bool(metadata, "startup_ok")
        if startup_ok is False:
            return None
        report = _metadata_json(metadata, "pixel_read_report")
        if report is None or not bool(report.get("pixel_read", pixel_read is True)):
            return None
        world_name = str(metadata.get("world_name", "") or metadata.get("package_name", "")).strip()
        world_id = str(metadata.get("world_id", "")).strip()
        if not world_name and not world_id:
            return None
        runtime_seed = metadata.get("seed", "")
        return {
            ...
            "asset_base_url": f"/output/package_exports/{normalized}/materialized/",
...
def api_create_live_session(access_code: str, request: PixelLiveSessionCreateRequest) -> dict[str, object]:
    ...
    normalized = _require_pixel_live_access_code(access_code)
    try:
