...
import json
def _render_headless_pixel_harness(
    seed: int,
    token: str,
    access_code: str = "",
    *,
    require_ai_reply: bool = True,
) -> str:
    ...
    payload = {
        ...
        "require_ai_reply": bool(require_ai_reply),
    }
    iframe_url = (
  ...
  <body>
    ...
    <script>
      (() => {
        ...
        const requireAiReply = __REQUIRE_AI_REPLY__;
        ...
        const captureFrameDataUrl = () => {
          try {
            const sourceCanvas = frame.contentWindow?.__AGORA_PHASER_GAME__?.canvas
              || frame.contentDocument?.querySelector("canvas")
              || null;
            if (!sourceCanvas || !sourceCanvas.width || !sourceCanvas.height) {
              return "";
            }
            const captureCanvas = document.createElement("canvas");
            captureCanvas.width = Number(sourceCanvas.width);
            captureCanvas.height = Number(sourceCanvas.height);
        ...
        const prepareCleanCanvasCapture = async () => {
          try {
            const scene = frame.contentWindow?.__AGORA_PHASER_GAME__?.scene?.scenes?.[0] || null;
            if (!scene) {
              return;
            }
            scene.captureMode = "visual_qa";
            [
              scene.liveUiController,
              scene.worldRenderer,
              scene.roomUiController,
              scene.povController,
            ].forEach((controller) => {
              if (controller) {
                controller.captureMode = "visual_qa";
              }
            });
            // Regression figures should document the player-facing embodied
            // view, not an atlas overview in which agents collapse to marks.
            scene.cameraController?.setViewMode?.("pov", { instant: true });
            scene.cameraController?.focusSelectedAgent?.({ instant: true });
            scene.worldRenderer?.applyRoomHighlight?.();
            const bubbles = scene.localPovState?.speechBubbles;
            if (
              bubbles
              && typeof bubbles.forEach === "function"
              && typeof bubbles.clear === "function"
            ) {
            ...
            // The canvas capture still provides useful evidence if cleanup is partial.
          }
        };
        ...
        const run = async () => {
          ...
          let agentReplyVisibleAt = messagePersistedAt;
          if (requireAiReply) {
            ...
            agentReplyVisibleAt = performance.now();
          }
          ...
          setStage("full page refresh");
          frame.contentWindow.location.reload();
          const refreshedSessionId = await waitFor(() => {
            const composer = frame.contentDocument?.querySelector("[data-live-send-message]");
            const movement = String(frame.contentDocument?.querySelector("#movement-module")?.innerText || "");
            if (!composer || !movement.includes("Live session")) {
              return null;
            }
            const candidate = parseSessionId(movement);
            return candidate && candidate !== sessionId ? candidate : null;
          }, 120000, 250);
          const refreshedState = await fetchLiveState(expectedCode, refreshedSessionId, 0);
          if (!String(refreshedState?.session?.claimed_agent_id || "").trim()) {
            throw new Error("Full page refresh did not restore a claimed live agent");
          }
          await prepareCleanCanvasCapture();
          const diagnostics = collectFrameDiagnostics();
          const screenshotDataUrl = captureFrameDataUrl();
          await postResult({
            status: "ok",
            seed,
            access_code: expectedCode,
            ...
            ai_reply_checked: requireAiReply,
            message_persist_ms: Math.round(messagePersistedAt - messageSendStartedAt),
            agent_reply_ms: requireAiReply ? Math.round(agentReplyVisibleAt - messageSendStartedAt) : null,
            initial_session_id: sessionId,
            refreshed_session_id: refreshedSessionId,
            refresh_triggered: true,
            before_hash: beforeHash,
            final_summary: summary.textContent,
            canvas_captured: Boolean(screenshotDataUrl),
    ...
    return (
        ...
        .replace("__REQUIRE_AI_REPLY__", json.dumps(payload["require_ai_reply"]))
        .replace("__IFRAME_URL__", iframe_url)
        .replace("__PIXEL_BUNDLE_VERSION__", pixel_bundle_version)
    )


def _render_pixel_live_snapshot(*, seed: int, access_code: str, session_id: str, token: str, label: str, expected_event_id: int = 0, focus_mode: str = "agent") -> str:
    payload = {
        "seed": int(seed),
        "access_code": str(access_code or "").strip(),
        ...
        "focus_mode": str(focus_mode or "agent").strip() or "agent",
    }
    iframe_url = (
        f"/pixel/?mode=live&seed={payload['seed']}"
  ...
  <body>
    ...
    <script>
      (() => {
        ...
        const focusReadableView = async (liveState) => {
          try {
            ...
            if (payload.focus_mode === "agent") {
              doc.getElementById("pov-mode-button")?.click();
              await sleep(650);
        ...
        const freezeFrameCanvas = () => {
          try {
            const sourceCanvas = frame.contentWindow?.__AGORA_PHASER_GAME__?.canvas
              || frame.contentDocument?.querySelector("canvas")
              || null;
            if (!sourceCanvas || !sourceCanvas.width || !sourceCanvas.height) {
              return { frozen: false, dataUrl: "" };
            }
            const frozenCanvas = document.createElement("canvas");
            frozenCanvas.width = Number(sourceCanvas.width);
            frozenCanvas.height = Number(sourceCanvas.height);
            frozenCanvas.style.width = "1600px";
            frozenCanvas.style.height = "900px";
            frozenCanvas.style.border = "1px solid #3b3142";
            frozenCanvas.style.background = "#0f0c13";
            ...
            context.imageSmoothingEnabled = false;
            context.drawImage(sourceCanvas, 0, 0);
            frame.replaceWith(frozenCanvas);
            return {
              frozen: true,
              dataUrl: frozenCanvas.toDataURL("image/png"),
            };
          } catch (error) {
            return { frozen: false, dataUrl: "" };
          }
        ...
        const run = async () => {
          try {
            ...
            await sleep(2000);
            const frozenFrame = freezeFrameCanvas();
            await postJson(`/__test__/headless-pixel/result/${payload.token}`, {
              ...
              canvas_frozen: frozenFrame.frozen,
              screenshot_data_url: frozenFrame.dataUrl,
              diagnostics,
            });
            window.stop();
          } catch (error) {
        ...
        const tryKickoff = () => {
          try {
            const doc = frame.contentDocument;
            const game = frame.contentWindow?.__AGORA_PHASER_GAME__ || null;
            const readyText = String(doc?.getElementById("event-status")?.textContent || "");
            if (
              game?.canvas
              || readyText.includes("Pixel UI ready")
              || doc?.getElementById("recenter-agent-button")
            ) {
              startRun();
            }
          } catch (error) {
            // Keep polling until the same-origin Pixel frame is readable.
          }
        };
        frame.addEventListener("load", tryKickoff, { once: true });
        kickoffTimer = setInterval(tryKickoff, 100);
        setTimeout(startRun, 5000);
      })();
    </script>
  </body>
</html>"""
