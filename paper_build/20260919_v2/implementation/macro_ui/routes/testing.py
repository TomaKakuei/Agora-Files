...
def api_headless_pixel_harness(
    seed: int = 42617,
    token: str = "",
    access_code: str = "",
...
def api_headless_pixel_gate(token: str) -> Response:
    ...
    gate.wait(timeout=max(30.0, _env_float("AGORA_HEADLESS_PIXEL_GATE_TIMEOUT_SECONDS", 180.0)))
    if not gate.is_set():
...
def api_headless_pixel_result(token: str, payload: dict[str, object]) -> dict[str, object]:
    ...
    if access_code:
        try:
            ...
            store.wait_for_background_idle(
                timeout_seconds=max(5.0, _env_float("AGORA_HEADLESS_PIXEL_BACKGROUND_IDLE_SECONDS", 45.0))
            )
