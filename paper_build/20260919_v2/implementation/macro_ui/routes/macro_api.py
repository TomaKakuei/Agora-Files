...
def _run_affordances(run_dir: Path) -> list[dict[str, Any]]:
    config_path = run_dir / "run_config.json"
    if not config_path.is_file():
        return []
    config = json.loads(config_path.read_text(encoding="utf-8"))
    actions = config.get("actions", {}) if isinstance(config, dict) else {}
    return [dict(item) for item in actions.get("interaction_affordance_catalog", []) if isinstance(item, dict)]
...
def api_world_builder_create_draft(request: WorldBuilderDraftCreateRequest) -> dict[str, object]:
    try:
        return world_builder.create_draft(serve_macro_ui.MACRO_PACKAGE_ROOT, request.model_dump())
...
def api_world_builder_get_draft(draft_id: str) -> dict[str, object]:
    try:
        return world_builder.get_draft_response(serve_macro_ui.MACRO_PACKAGE_ROOT, draft_id)
...
def api_world_builder_resolve(identifier: str = "", world_name: str = "", world_id: str = "") -> dict[str, object]:
    try:
        return world_builder.resolve_draft(
            serve_macro_ui.MACRO_PACKAGE_ROOT,
...
def api_world_builder_revise_draft(draft_id: str, request: WorldBuilderDraftReviseRequest) -> dict[str, object]:
    ...
    try:
        return world_builder.revise_draft(serve_macro_ui.MACRO_PACKAGE_ROOT, draft_id, str(request.feedback))
    except FileNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc)) from exc
...
def api_world_builder_draft_package(draft_id: str) -> FileResponse:
    try:
        package_path = world_builder.draft_package_path(serve_macro_ui.MACRO_PACKAGE_ROOT, draft_id)
    except FileNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc)) from exc
...
def api_world_builder_launch_art(draft_id: str) -> dict[str, object]:
    try:
        return {
            "draft_id": draft_id,
            "art": world_builder.launch_art_worker(serve_macro_ui.MACRO_PACKAGE_ROOT, draft_id),
        }
    except FileNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except Exception as exc:
...
def api_world_builder_art_status(draft_id: str) -> dict[str, object]:
    try:
        return world_builder.art_status(serve_macro_ui.MACRO_PACKAGE_ROOT, draft_id)
    except FileNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc)) from exc
...
def api_world_builder_publish(draft_id: str) -> dict[str, object]:
    try:
        return world_builder.publish_draft(serve_macro_ui.MACRO_PACKAGE_ROOT, draft_id)
    except FileNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
def api_world_builder_history(draft_id: str) -> dict[str, object]:
    try:
        return world_builder.draft_history(serve_macro_ui.MACRO_PACKAGE_ROOT, draft_id)
    except FileNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc)) from exc
...
@router.get("/runs/{run_id}/affordances")
def api_run_affordances(run_id: str) -> dict[str, object]:
    run_dir = _run_dir_for_id(run_id)
    return {"run_id": run_id, "affordances": _run_affordances(run_dir)}
...
def api_human_action(run_id: str, request: HumanActionRequest) -> dict[str, object]:
    run_dir = _run_dir_for_id(run_id)
    paths = _human_paths(run_dir)
    affordance_id = str(request.affordance_id or "").strip()
    if affordance_id:
        known = {str(item.get("affordance_id", "")) for item in _run_affordances(run_dir)}
        if affordance_id not in known:
            raise HTTPException(status_code=400, detail=f"Unknown affordance_id for this world: {affordance_id}")
    presence_payload = {
    ...
    event = {
        ...
        "open_action_proposal": dict(request.open_action_proposal) if isinstance(request.open_action_proposal, dict) else None,
