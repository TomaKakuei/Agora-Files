...
def _image_client_for_config(config: dict[str, Any]) -> VertexInlineImageClient:
    ...
    endpoint = str(
        config.get("macro_ui", {}).get("flux_endpoint")
        or config.get("image_generation", {}).get("endpoint")
...
DEFAULT_PY_BIN = Path(
    ...
    or os.environ.get("AGORA_PYTHON_BIN")
    or sys.executable
)
...
class VertexInlineImageClient:
    """Compatibility image client backed by the configured FLUX HTTP service."""
    ...
    def __init__(
        ...
        *,
        ...
        self.api_key = str(os.environ.get(api_key_env, "")).strip() if api_key_env else ""
    ...
    def generate_image(self, *, prompt: str, output_path: Path) -> dict[str, Any]:
        resolved_output = output_path.with_suffix(".png")
        resolved_output.parent.mkdir(parents=True, exist_ok=True)
        payload = {
            "prompt": prompt,
            "negative_prompt": "text, watermark, blurry, low quality, malformed hands, extra limbs",
            "width": 1024,
            "height": 1024,
            "steps": 8,
            "guidance_scale": 0.0,
        ...
        for attempt in range(1, attempts + 1):
            try:
                response = requests.post(f"{self.endpoint_base}/generate", json=payload, timeout=self.timeout_seconds)
                response.raise_for_status()
                result = response.json()
                if isinstance(result.get("image_base64"), str) and result["image_base64"].strip():
                    resolved_output.write_bytes(base64.b64decode(result["image_base64"]))
                elif not resolved_output.is_file() and isinstance(result.get("image_path"), str):
                    generated_path = Path(result["image_path"])
                    if generated_path.is_file():
                        resolved_output.write_bytes(generated_path.read_bytes())
                if not resolved_output.is_file():
                    raise RuntimeError("FLUX macro image generation returned no usable image.")
                return {"image_path": str(resolved_output), "image_mime_type": "image/png", "text": ""}
            except Exception as exc:
                last_error = exc
                if attempt < attempts:
                    ...
                    continue
        raise RuntimeError(f"FLUX image generation failed: {last_error}")
