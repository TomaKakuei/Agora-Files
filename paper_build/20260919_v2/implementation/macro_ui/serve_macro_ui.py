...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--reload",
        action="store_true",
        default=str(os.environ.get("AGORA_MACRO_UI_RELOAD", "")).strip().lower() in {"1", "true", "yes", "on"},
        help="Enable uvicorn auto-reload for local UI development. Keep this off for live/load tests.",
    )
...
app.mount(
    "/output",
    StaticFiles(directory=str(PACKAGE_ROOT / "output"), check_dir=False),
    name="output-static",
)
...
def main() -> None:
    ...
    uvicorn.run(
        ...
        reload_dirs=[
            str(PACKAGE_ROOT / "agora_ui"),
            str(PACKAGE_ROOT / "macro_ui"),
            str(PACKAGE_ROOT / "frontend" / "src"),
            str(PACKAGE_ROOT / "world_creator_ui"),
        ] if bool(args.reload) else None,
    )
