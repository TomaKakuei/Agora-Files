...
def export_world_package_from_db(
    *,
    ...
    meta_updates: dict[str, Any] = {
        ...
        "pixel_read_report": json.dumps(pixel_report, ensure_ascii=False),
        "created_at": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        # Keep incomplete exports out of the public catalog. Direct access by
        # code still works for the launch probe below.
        "validation_probe": "1",
        "validation_probe_created_at": datetime.now(timezone.utc).isoformat(timespec="seconds"),
    }
    if extra_meta:
