...
def architecture_figure(output_root: Path) -> None:
    ...
    fig.savefig(output_root / "one_sentence_pipeline.pdf", bbox_inches="tight")
    fig.savefig(output_root / "one_sentence_pipeline.png", bbox_inches="tight")
    plt.close(fig)
...
def results_figure(root: Path, output_root: Path) -> None:
    ...
    output_root.mkdir(parents=True, exist_ok=True)
    fig.savefig(output_root / "objective_results.pdf", bbox_inches="tight")
    fig.savefig(output_root / "objective_results.png", bbox_inches="tight")
    plt.close(fig)
...
def world_strip_figure(root: Path, output_root: Path) -> None:
    ...
    output_root.mkdir(parents=True, exist_ok=True)
    fig.savefig(output_root / "five_world_strip.pdf", bbox_inches="tight")
    fig.savefig(output_root / "five_world_strip.png", bbox_inches="tight")
    plt.close(fig)
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("docs/figures_20260729"),
    )
