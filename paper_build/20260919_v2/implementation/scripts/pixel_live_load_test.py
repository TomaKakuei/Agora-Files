...
def run_load_test(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if bool(args.skip_catalog) and str(args.access_code).strip():
            access_code = str(args.access_code).strip()
            worlds = []
            world = {
                "access_code": access_code,
                "world_name": str(args.world_name or access_code).strip(),
                "seed": args.seed,
                "live_ws_url_template": f"/api/pixel/worlds/{access_code}/live/ws/{{session_id}}",
            }
        else:
        ...
        if str(args.transport) == "ws-movement":
            ...
            for session in sessions:
                ...
                if not bool(args.disable_actions):
                    threads.append(
                        threading.Thread(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--skip-catalog", action=argparse.BooleanOptionalAction, default=False)
