...
def _read_json(url: str) -> dict[str, Any]:
    timeout_seconds = max(15.0, _env_float("AGORA_HEADLESS_HTTP_TIMEOUT_SECONDS", 60.0))
    try:
        with urllib.request.urlopen(url, timeout=timeout_seconds) as response:
            return json.loads(response.read().decode("utf-8"))
    except Exception as exc:
        raise RuntimeError(f"JSON request failed for {url} after {timeout_seconds:.1f}s: {exc}") from exc
...
def _capture_snapshot(
    *,
    ...
    try:
        try:
            snapshot_result = _wait_for_result(base_url, token, timeout_s=timeout_s)
            screenshot_data_url = str(snapshot_result.get("screenshot_data_url", "")).strip()
            if screenshot_data_url.startswith("data:image/png;base64,"):
                screenshot_path.parent.mkdir(parents=True, exist_ok=True)
                screenshot_path.write_bytes(base64.b64decode(screenshot_data_url.split(",", 1)[1]))
        except Exception as exc:
            snapshot_warning = f"snapshot harness did not finish cleanly: {exc}"
    finally:
        if snapshot_result and browser.poll() is None:
            browser.terminate()
        try:
            browser_stdout, browser_stderr = browser.communicate(timeout=min(15.0, timeout_s))
        except Exception:
...
def _verify_refresh_session(
    ...
    with sqlite3.connect(live_db) as conn:
        ...
        if refreshed_session_id and str(active["session_id"]) != str(refreshed_session_id):
            raise RuntimeError(
                f"active refresh session mismatch: expected {refreshed_session_id}, got {active['session_id']}"
            )
    ...
    with sqlite3.connect(live_db) as conn:
        ...
        if require_ai_reply:
            ...
            if response_event is None:
                if str(human_payload.get("message_status", "")).strip() != "completed":
                    raise RuntimeError("AI reply event did not land in the live DB")
                response_payload = human_payload
            else:
        ...
        if require_ai_reply:
            ...
            if has_target_agent:
                ...
                if response_event is not None and str(target_state.get("last_ai_response_text", "")).strip() != str(response_event["response_text"] or "").strip():
                    raise RuntimeError("target state_json did not persist last_ai_response_text")
            if response_event is not None and str(actor_state.get("last_ai_response_text", "")).strip() != str(response_event["response_text"] or "").strip():
        ...
        return {
            ...
            "response_event_id": int(response_event["event_id"]) if response_event is not None else None,
            ...
            "ai_reply_checked": bool(require_ai_reply),
...
def run(
    ...
    snapshot_timeout_seconds: float = 75.0,
    require_ai_reply: bool = True,
) -> int:
    ...
    for attempt_index in range(max(1, int(max_attempts))):
        status = _run_once(
            ...
            require_ai_reply=require_ai_reply,
        )
        if status == 0:
            return 0
...
def _run_once(
    ...
    try:
        ...
        harness_url = (
            ...
            f"&require_ai_reply={1 if require_ai_reply else 0}"
        )
        ...
        result = None
        try:
            result = _wait_for_result(base_url, token, timeout_s=max(60.0, float(timeout_seconds)))
        finally:
            if browser.poll() is None:
                browser.terminate()
                try:
        ...
        refreshed_session_id = str(result.get("refreshed_session_id", "")).strip()
        if not bool(result.get("refresh_triggered")) or not refreshed_session_id:
            raise RuntimeError("full page refresh was not completed")
        refresh_session = _verify_refresh_session(
            expected_code,
            initial_session_id,
        ...
        screenshot_data_url = str(result.pop("screenshot_data_url", "")).strip()
        screenshot_source = "main_harness_canvas"
        screenshot_warning = ""
        if screenshot_data_url.startswith("data:image/png;base64,"):
            screenshot_path.parent.mkdir(parents=True, exist_ok=True)
            screenshot_path.write_bytes(base64.b64decode(screenshot_data_url.split(",", 1)[1]))
        if not screenshot_path.is_file() or screenshot_path.stat().st_size <= 0:
            screenshot_source = "snapshot_harness_fallback"
            screenshot_warning = _capture_snapshot(
                ...
                timeout_s=max(30.0, float(snapshot_timeout_seconds)),
            )
        if not screenshot_path.is_file() or screenshot_path.stat().st_size <= 0:
            raise RuntimeError(
                f"Pixel harness produced no screenshot: {screenshot_warning or screenshot_path}"
            )
        print(json.dumps(
            {
                "status": "ok",
                ...
                "validation_profile": "full_ai_reply" if require_ai_reply else "deterministic_publish",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--skip-ai-reply",
        action="store_true",
        help="Validate deterministic UI and persistence contracts without waiting for an external model reply.",
    )
    parser.add_argument(
        "--timeout-seconds",
        type=float,
        default=_env_float("AGORA_PIXEL_RESULT_TIMEOUT_SECONDS", 480.0),
        help="Seconds to wait for the main Pixel harness result.",
    )
    parser.add_argument(
        "--snapshot-timeout-seconds",
        type=float,
        default=_env_float("AGORA_PIXEL_SNAPSHOT_TIMEOUT_SECONDS", 120.0),
        help="Seconds to wait for the final snapshot harness.",
    )
    ...
    try:
        raise SystemExit(
            run(
                ...
                snapshot_timeout_seconds=max(30.0, float(args.snapshot_timeout_seconds)),
                require_ai_reply=not bool(args.skip_ai_reply),
            )
        )
    except Exception as exc:
