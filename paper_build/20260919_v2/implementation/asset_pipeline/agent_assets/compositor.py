...
def _reuse_latest_raw_sheet(package_root: Path, agent_id: str, revision: str, destination: Path) -> dict[str, Any]:
    ...
    if source.resolve() == destination.resolve():
        return {
            "status": "reused_latest_raw_sheet",
            "source": str(source),
            "mode": "reused_latest_raw_sheet",
        }
...
def _build_atlas_outputs(
    *,
    ...
    atlas = build_phaser_atlas(
        ...
        target_frame_width=processing.get("target_frame_width", 64),
        target_frame_height=processing.get("target_frame_height", 64),
...
def _build_ai_studio_retry_world_config(base_config: dict[str, Any]) -> dict[str, Any]:
    ...
    image_generation.pop("response_modalities", None)
...
def _build_ai_studio_retry_pipeline_config(base_pipeline_config: dict[str, Any]) -> dict[str, Any]:
    ...
    sprite_generation["adapter"] = "flux_local_service"
    sprite_generation.setdefault("endpoint", image_generation["endpoint"])
