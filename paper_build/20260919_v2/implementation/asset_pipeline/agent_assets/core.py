...
from asset_pipeline.sprite_qa_programmatic import directional_sheet_qa, final_atlas_transparency_qa
...
def parse_args(args=None):
    ...
    parser.add_argument('--bootstrap-procedural-sheet', action='store_true', help='Deprecated; procedural sprite generation is disabled by the strict pipeline.')
...
def run_pipeline(parsed_args):
    if parsed_args.bootstrap_procedural_sheet:
        raise RuntimeError(
            "Procedural sprite bootstrap is disabled: strict world generation requires "
            "a remote-generated or explicitly supplied sheet that passes QA."
        )
    ...
    if raw_sheet_path.exists():
        ...
        if not qa_report.get("pass_qa", False):
            ...
            sys.exit(1)
        direction_qa = directional_sheet_qa(raw_sheet_path)
        qa_report["directional_rows"] = direction_qa
        qa_report["pass_qa"] = bool(qa_report.get("pass_qa", False) and direction_qa.get("pass", False))
        _write_json(asset_dir / "raw_sheet_quality_report.json", qa_report)
        if not direction_qa.get("pass", False) and not parsed_args.raw_sheet:
            print(
                f"CRITICAL: Directional hard QA failed for {parsed_args.agent_id}: {direction_qa}",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        atlas_outputs = _build_atlas_outputs(
            ...
            quality_report_path=asset_dir / "atlas_quality_report.json",
            sheet_layout=sheet_layout,
            processing=processing,
            animation_states=animation_states
        )
        atlas_quality_report = atlas_outputs[1]
        atlas_quality_pass = bool(
            atlas_quality_report.get("quality_summary", {}).get("pass", False)
        )
        if not atlas_quality_pass:
            print(
                f"CRITICAL: Atlas frame-integrity QA failed for {parsed_args.agent_id}: "
                f"{atlas_quality_report.get('quality_summary', {})}",
                file=sys.stderr,
            )
            sys.exit(1)
        qa_report["atlas_quality_summary"] = dict(
            atlas_quality_report.get("quality_summary", {})
        )
...
def _write_asset_bundle_and_event(
    ...
    bundle = AssetBundleSpec.model_validate({
        ...
        "atlas_quality_report_path": str(asset_dir / "atlas_quality_report.json"),
