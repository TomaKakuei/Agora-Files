...
def _draw_grass_tile(
    ...
    *,
    ...
    if bool(world_terrain.get("minimal_terrain", False)):
        minimal_accent = str(world_terrain.get("grass_leaf_hex", alt))
        fill = _mix(base, minimal_accent, 0.05 + 0.04 * noise)
        draw.rectangle(tile_box, fill=fill)
        draw.line((left, top, right, top), fill=_mix(base, minimal_accent, 0.12), width=1)
        draw.line((left, bottom, right, bottom), fill=_mix(base, shadow, 0.08), width=1)
        if noise > 0.97:
            draw.rectangle((left + 5, top + 5, left + 7, top + 7), fill=leaf)
        return
    draw.rectangle(tile_box, fill=fill)
...
def _draw_path_tile(
    ...
    *,
    ...
    if bool(world_terrain.get("minimal_terrain", False)):
        terrain_shadow = str(world_terrain.get("grass_shadow_hex", shadow_hex))
        route_color = str(world_terrain.get("grass_leaf_hex", base_hex))
        fill = _mix(
            terrain_shadow,
            route_color,
            0.42 + 0.06 * _stable_noise(tile_x, tile_y, seed=11),
        )
        draw.rectangle(tile_box, fill=fill)
        draw.line((left, top, right, top), fill=_mix(terrain_shadow, route_color, 0.58), width=1)
        draw.line((left, bottom, right, bottom), fill=_mix(terrain_shadow, shadow_hex, 0.24), width=1)
        return
    ...
    *,
    ...
    if bool(world_terrain.get("minimal_terrain", False)):
        ...
        fill = _mix(
            terrain_shadow,
            route_color,
            0.58 + 0.06 * _stable_noise(tile_x, tile_y, seed=7),
        )
        draw.rectangle(tile_box, fill=fill)
        draw.line((left, top, right, top), fill=_mix(terrain_shadow, route_color, 0.70), width=1)
        draw.line((left, bottom, right, bottom), fill=_mix(terrain_shadow, shadow_hex, 0.22), width=1)
        return
...
def render_component_icon(
    *,
    ...
    detail_radius = max(2, icon_px // 24)
    center = icon_px // 2
    draw.rectangle(
        (
            center - detail_radius,
            center - detail_radius,
            center + detail_radius,
            center + detail_radius,
        ),
...
def _component_box(
    ...
    *,
    ...
    if bool(component_spec.get("semantic_generated", False)):
        # FLUX component artifacts are normalized to square icon canvases. A
        # skinny authored footprint would otherwise shrink the whole icon to
        # one unreadable tile when ImageOps.contain preserves that canvas.
        semantic_extent = max(width, height, tile_px * 3 - 10)
        room_width = max(1, room_box[2] - room_box[0])
        room_height = max(1, room_box[3] - room_box[1])
        inner_pad = max(6, tile_px // 4) * 2
        width = min(semantic_extent, max(tile_px, room_width - inner_pad))
        height = min(semantic_extent, max(tile_px, room_height - inner_pad))
...
def _component_readability_metrics(
    *,
    icon_path: Path,
    box: tuple[int, int, int, int],
    room_box: tuple[int, int, int, int],
    ...
    room_area = max(1, (room_box[2] - room_box[0]) * (room_box[3] - room_box[1]))
    rendered_width = 0
    rendered_height = 0
    opaque_fraction = 0.0
    if icon_path.is_file():
        with Image.open(icon_path) as source:
            ...
            if bbox is not None:
                icon = icon.crop(bbox)
                scale = min(box_width / max(1, icon.width), box_height / max(1, icon.height))
                rendered_width = max(1, round(icon.width * scale))
                rendered_height = max(1, round(icon.height * scale))
                alpha = icon.getchannel("A")
                opaque_pixels = sum(alpha.histogram()[8:])
                opaque_fraction = opaque_pixels / max(1, icon.width * icon.height)
    footprint_ratio = (rendered_width * rendered_height) / room_area
    effective_opaque_ratio = (
        rendered_width * rendered_height * opaque_fraction
    ) / room_area
    minimum_short_edge = max(40, round(tile_px * 1.5))
    readability_pass = bool(
        min(rendered_width, rendered_height) >= minimum_short_edge
        and footprint_ratio >= 0.01
        and effective_opaque_ratio >= 0.0025
    )
    return {
        "rendered_size_pixels": [rendered_width, rendered_height],
        "room_footprint_ratio": round(footprint_ratio, 6),
        "effective_opaque_ratio": round(effective_opaque_ratio, 6),
        "minimum_short_edge_pixels": minimum_short_edge,
...
def _room_component_placements(
    ...
    *,
    ...
    for component_id in room.get("visual", {}).get("decor_tags", []):
        ...
        requested.append((component_id, spec, anchor))
        seen.add((component_id, anchor))
    ...
    # Authored semantic props retain their anchors. Generic supporting props
    # fill remaining space and relocate deterministically instead of covering
    # the world-specific visual evidence.
    requested.sort(key=lambda item: 0 if bool(item[1].get("semantic_generated", False)) else 1)
    anchor_order = (
        "center",
        "north_west",
        "north_east",
    ...
    clearance = max(2, tile_px // 8)
    for component_id, spec, authored_anchor in requested:
        semantic = bool(spec.get("semantic_generated", False))
        candidate_anchors = (
            authored_anchor,
            *(anchor for anchor in anchor_order if anchor != authored_anchor),
        )
        ...
        for anchor in candidate_anchors:
            ...
            if not any(
                _boxes_overlap(candidate_box, existing, gap=clearance)
        ...
        if selected_box is None:
            if not semantic:
                continue
            selected_box = _component_box(
                spec,
                room_box=room_box,
                tile_px=tile_px,
                anchor_override=authored_anchor,
            )
        placements.append((component_id, spec, selected_box))
...
def render_map_asset(
    *,
    ...
    overlay_supporting_props: bool = True,
    ...
    terrain_override = dict(world_terrain) if isinstance(world_terrain, dict) else {}
    custom_terrain = _merge_world_terrain(None)
    ...
    custom_terrain.update(terrain_override)
    if terrain_override:
        custom_terrain.setdefault("minimal_terrain", True)
    ...
    # 2. Place semantic room components after the generated room plate so they
    # remain legible and traceable to the typed room IR.
    resolved_component_library = _merge_component_library(component_library)
    resolved_component_icons = component_icons if isinstance(component_icons, dict) else {}
    component_placements: list[dict[str, Any]] = []
    for room in rooms:
        room_x = int(room.get("x_pos", room.get("x", 0)))
        room_y = int(room.get("y_pos", room.get("y", 0)))
        room_width = int(room.get("width_tiles", 1))
        room_height = int(room.get("height_tiles", 1))
        room_box = (
            margin_px + room_x * tile_px,
            margin_px + room_y * tile_px,
            margin_px + (room_x + room_width) * tile_px,
            margin_px + (room_y + room_height) * tile_px,
        )
        for component_id, component_spec, component_box in _room_component_placements(
            room,
            component_library=resolved_component_library,
            tile_px=tile_px,
            room_box=room_box,
        ):
            if not overlay_supporting_props and not bool(component_spec.get("semantic_generated", False)):
                continue
            icon_value = str(resolved_component_icons.get(component_id, "")).strip()
            icon_pasted = bool(
                icon_value
                and _paste_component_icon(image, icon_path=Path(icon_value), box=component_box)
            )
            readability = (
                _component_readability_metrics(
            ...
            component_placements.append(
                {
                    ...
                    "box": list(component_box),
                    "semantic_generated": bool(component_spec.get("semantic_generated", False)),
            ...
            if icon_pasted:
                continue
            if bool(component_spec.get("semantic_generated", False)):
                raise RuntimeError(f"Missing generated semantic component icon: {component_id}")
            _draw_component_preset(
                draw,
                render_name=str(component_spec.get("render", "quest_notice")),
                box=component_box,
                color="#352f3b",
                accent="#d7b66f",
            )

    # Grid lines are a debugging aid. They are disabled in presentation builds.
    ...
    doorway_wall_keys: set[tuple[int, int, str]] = set()
    for room in rooms:
        for doorway in room.get("doorways", []):
            position = doorway.get("position", {}) if isinstance(doorway.get("position", {}), dict) else {}
            door_x = int(position.get("x", 0))
            door_y = int(position.get("y", 0))
            edge_dir = str(doorway.get("edge_dir", "")).strip().lower()
            if edge_dir == "right":
                doorway_wall_keys.add((door_x, door_y, "right"))
            elif edge_dir == "left":
                doorway_wall_keys.add((door_x - 1, door_y, "right"))
            elif edge_dir == "bottom":
                doorway_wall_keys.add((door_x, door_y, "bottom"))
            elif edge_dir == "top":
                doorway_wall_keys.add((door_x, door_y - 1, "bottom"))

    # Thin walls (passable) - 2px
    for wall in thin_walls:
        if (int(wall["x"]), int(wall["y"]), str(wall["dir"])) in doorway_wall_keys:
            continue
    ...
    output_path.with_suffix(".components.json").write_text(
        json.dumps(
            {
                "schema_version": "agora.map_component_placements.v2",
                "map_path": str(output_path),
                "map_size": [canvas_w, canvas_h],
