...
def materialize_scenario(
    ...
    *,
    ...
    custom_actions = list(actions_config.get("allowed_custom_actions", []))
    for affordance in actions_config.get("interaction_affordance_catalog", []):
        if not isinstance(affordance, dict) or str(affordance.get("kind", "")) not in {"custom", "cinematic"}:
            continue
        label = str(affordance.get("label", "")).strip()
        if label and label not in custom_actions:
            custom_actions.append(label)
    ...
    world_rules = {
        ...
        "custom_action_rules": {
            ...
            "open_proposals_enabled": bool(custom_rule_config.get("open_proposals_enabled", True)),
            "open_proposal_effects": [
                str(value)
                for value in custom_rule_config.get(
                    "open_proposal_effects",
