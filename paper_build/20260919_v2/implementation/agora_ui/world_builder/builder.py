...
def _ordinary_routes(
    *,
    ...
    for index, loop in enumerate([dict(entry) for entry in builder_spec.get("gameplay_loops", []) if isinstance(entry, dict)], start=1):
        ...
        routes.append(route)
    existing_actions = {str(route.get("action", "")).strip().lower() for route in routes}
    generic_actions = {"chat", "inspect", "coordinate", "trade", "move", "cinematicinteraction"}
    for index, raw_action in enumerate(builder_spec.get("custom_actions", []), start=1):
        action = str(raw_action).strip()
        if not action or action.lower() in generic_actions or action.lower() in existing_actions:
            continue
        routes.append(
            {
                "route_id": _slug(f"world_action_{action}_{index}"),
                "kind": "custom",
                "action": action,
                "status_effect": _slug(f"{action}_advanced"),
                "duration_steps": 2,
                "weight": 10,
                "story_verb": _route_story_verb(action),
...
def _interaction_affordance_catalog(routes: list[dict[str, Any]]) -> list[dict[str, Any]]:
    catalog: list[dict[str, Any]] = []
    for route in routes:
        route_id = str(route.get("route_id", "")).strip()
        if not route_id:
            continue
        kind = str(route.get("kind", "custom")).strip() or "custom"
        action = str(route.get("player_label", route.get("action", ""))).strip()
        if not action:
            action = {
                "item_trade": "Trade",
                "move": "Move",
                "image": "Document",
                "cinematic": "Act Together",
            }.get(kind, "Interact")
        same_room = kind in {"custom", "item_trade", "cinematic"}
        persistent_effects = [
            effect
            for effect, enabled in (
                ("relationship", kind in {"custom", "item_trade", "cinematic"}),
                ("inventory", kind == "item_trade"),
                ("location", kind == "move"),
                ("artifact", kind == "image"),
                ("status", bool(route.get("status_effect"))),
            )
            if enabled
        ]
        catalog.append(
            {
                ...
                "route_id": route_id,
                "label": action,
                "description": str(route.get("selection_guidance", route.get("story_verb", action))).strip(),
                "kind": kind,
                "interaction_family": {
                    "item_trade": "economy",
                    "move": "exploration",
                    "image": "creation",
                    "cinematic": "embodied_social",
                }.get(kind, "social"),
                "actor_modes": ["human", "ai"],
                "target_types": ["room"] if kind == "move" else (["world"] if kind == "image" else ["agent", "human_player"]),
                "preconditions": {
                    "same_room": same_room,
                    "actor_role_ids": list(route.get("actor_role_ids", [])),
                    "requires_actor_item_ids_any": list(route.get("requires_actor_item_ids_any", [])),
                },
                "persistent_effects": persistent_effects,
                "status_effect": str(route.get("status_effect", "")),
                "evidence_fields": ["actor_id", "route_id", "success", "round_index"]
                + (["target_id", "relationship_adjustments"] if kind in {"custom", "item_trade", "cinematic"} else [])
                + (["trade_item_id"] if kind == "item_trade" else [])
                + (["actor_room_id", "target_room_id"] if kind == "move" else []),
            }
        )
    return catalog
...
def _build_world_config_from_spec(
    ...
    *,
    ...
    pixel_pipeline = config.setdefault("pixel_asset_pipeline", {})
    pixel_frontend = pixel_pipeline.setdefault("frontend", {})
    map_generation = pixel_pipeline.setdefault("map_generation", {})
    agent_visual_policy = dict(builder_spec.get("agent_visual_policy", {})) if isinstance(builder_spec.get("agent_visual_policy", {}), dict) else {}
    if agent_visual_policy:
        pixel_pipeline["agent_visual_policy"] = agent_visual_policy
    visual_canon = dict(builder_spec.get("visual_canon", {})) if isinstance(builder_spec.get("visual_canon", {}), dict) else {}
    if visual_canon:
        pixel_pipeline["visual_canon"] = visual_canon
        scenario_meta["visual_canon_hash"] = str(visual_canon.get("canon_hash", ""))
        map_generation["margin_px"] = 0
        map_generation["show_grid_overlay"] = False
        map_generation["overlay_supporting_props"] = True
        map_generation["supporting_prop_policy_version"] = "collision_aware_v2"
        canon_palette = dict(visual_canon.get("palette", {}))
        map_generation["background_hex"] = str(canon_palette.get("shadow", "#171a22"))
        map_generation["world_terrain"] = {
            "grass_base_hex": str(canon_palette.get("shadow", "#171a22")),
            "grass_alt_hex": str(canon_palette.get("secondary", "#71859a")),
            "grass_shadow_hex": str(canon_palette.get("shadow", "#171a22")),
            "grass_leaf_hex": str(canon_palette.get("secondary", "#71859a")),
            "grass_flower_hex": str(canon_palette.get("accent", "#d8a44c")),
            "path_base_hex": str(canon_palette.get("secondary", "#71859a")),
            "path_highlight_hex": str(canon_palette.get("highlight", "#e9edf2")),
            "path_shadow_hex": str(canon_palette.get("shadow", "#171a22")),
            "courtyard_base_hex": str(canon_palette.get("secondary", "#71859a")),
            "courtyard_highlight_hex": str(canon_palette.get("accent", "#d8a44c")),
            "courtyard_shadow_hex": str(canon_palette.get("shadow", "#171a22")),
            "minimal_terrain": True,
        }
        component_generation = map_generation.setdefault("component_generation", {})
        semantic_component_budget = min(
            64,
            max(12, 4 * len(space.get("rooms", []))),
        )
        component_generation.update(
            {
                "enabled": True,
                "backend": "flux_local_service",
                "max_jobs": semantic_component_budget,
                "hard_max_jobs": 64,
                "width": 384,
                "height": 384,
                "steps": 4,
                "guidance_scale": 0.0,
                "timeout_seconds": 600,
                "strict_semantic_components": True,
            }
        )
    ...
    space["grid_shape"] = {
        "x": int(space["width_tiles"]),
        "y": int(space["height_tiles"]),
        "z": 1,
    }
    ...
    for index, room_spec in enumerate(source_room_defs):
        ...
        visual["visual_canon_hash"] = str(visual_canon.get("canon_hash", ""))
        ...
        for component_index, raw_component in enumerate(room_spec.get("scene_components", []), start=1):
            if not isinstance(raw_component, dict):
                continue
            base_component_id = _slug(
                _first_non_empty(
                    raw_component.get("component_id"),
                    raw_component.get("label"),
                    default=f"hero_prop_{component_index}",
                )
            )
            scene_components.append(
                {
                    "component_id": f"{room_id}_{base_component_id}"[:128],
                    "label": _first_non_empty(raw_component.get("label"), default=base_component_id.replace("_", " ").title()),
                    "description": _first_non_empty(raw_component.get("description"), default=""),
                    "anchor": _first_non_empty(raw_component.get("anchor"), default="center"),
                    "width_tiles": min(3, max(1, int(raw_component.get("width_tiles", 1) or 1))),
                    "height_tiles": min(3, max(1, int(raw_component.get("height_tiles", 1) or 1))),
                }
            )
        if visual_canon and not scene_components:
            raise ValueError(f"Room {room_name} lost its semantic scene components during compilation.")
        visual["scene_components"] = scene_components
    ...
    for left_index, left_room in enumerate(updated_rooms):
        for right_room in updated_rooms[left_index + 1:]:
            left_x = int(left_room["x"])
            left_y = int(left_room["y"])
            left_w = int(left_room["width_tiles"])
            left_h = int(left_room["height_tiles"])
            right_x = int(right_room["x"])
            right_y = int(right_room["y"])
            right_w = int(right_room["width_tiles"])
            right_h = int(right_room["height_tiles"])
            doorway_pair: tuple[dict[str, Any], dict[str, Any]] | None = None

            overlap_y_start = max(left_y, right_y)
            overlap_y_end = min(left_y + left_h, right_y + right_h)
            if overlap_y_end - overlap_y_start >= 3 and left_x + left_w == right_x:
                door_y = (overlap_y_start + overlap_y_end - 1) // 2
                doorway_pair = (
                    {"position": {"x": left_x + left_w - 1, "y": door_y, "z": 0}, "edge_dir": "right"},
                    {"position": {"x": right_x, "y": door_y, "z": 0}, "edge_dir": "left"},
                )
            elif overlap_y_end - overlap_y_start >= 3 and right_x + right_w == left_x:
                door_y = (overlap_y_start + overlap_y_end - 1) // 2
                doorway_pair = (
                    {"position": {"x": left_x, "y": door_y, "z": 0}, "edge_dir": "left"},
                    {"position": {"x": right_x + right_w - 1, "y": door_y, "z": 0}, "edge_dir": "right"},
                )

            overlap_x_start = max(left_x, right_x)
            overlap_x_end = min(left_x + left_w, right_x + right_w)
            if doorway_pair is None and overlap_x_end - overlap_x_start >= 3 and left_y + left_h == right_y:
                door_x = (overlap_x_start + overlap_x_end - 1) // 2
                doorway_pair = (
                    {"position": {"x": door_x, "y": left_y + left_h - 1, "z": 0}, "edge_dir": "bottom"},
                    {"position": {"x": door_x, "y": right_y, "z": 0}, "edge_dir": "top"},
                )
            elif doorway_pair is None and overlap_x_end - overlap_x_start >= 3 and right_y + right_h == left_y:
                door_x = (overlap_x_start + overlap_x_end - 1) // 2
                doorway_pair = (
                    {"position": {"x": door_x, "y": left_y, "z": 0}, "edge_dir": "top"},
                    {"position": {"x": door_x, "y": right_y + right_h - 1, "z": 0}, "edge_dir": "bottom"},
                )

            if doorway_pair is None:
                continue
            doorway_id = f"{left_room['room_id']}_to_{right_room['room_id']}"[:180]
            left_room["doorways"].append(
                {
                    "doorway_id": doorway_id,
                    **doorway_pair[0],
                    "connects_to_room_id": right_room["room_id"],
                    "label": f"passage to {right_room['name']}",
                }
            )
            right_room["doorways"].append(
                {
                    "doorway_id": doorway_id,
                    **doorway_pair[1],
                    "connects_to_room_id": left_room["room_id"],
                    "label": f"passage to {left_room['name']}",
                }
            )

    component_library = map_generation.setdefault("component_library", {})
    component_props = component_library.setdefault("props", {})
    room_layout_presets = component_library.setdefault("room_layout_presets", {})
    for room in updated_rooms:
        supplemental_props: list[dict[str, str]] = []
        for component in room.get("visual", {}).get("scene_components", []):
            component_id = str(component.get("component_id", "")).strip()
            if not component_id:
                continue
            component_props[component_id] = {
                "render": "table_scroll",
                "anchor": str(component.get("anchor", "center")),
                "size_tiles": {
                    "w": int(component.get("width_tiles", 1)),
                    "h": int(component.get("height_tiles", 1)),
                },
                "label": str(component.get("label", component_id)),
                "description": str(component.get("description", "")),
                "semantic_generated": True,
                "visual_canon_hash": str(visual_canon.get("canon_hash", "")),
            }
            supplemental_props.append(
                {
        ...
        if supplemental_props:
            room_layout_presets[room["room_id"]] = {"supplemental_props": supplemental_props}
    ...
    map_visual["visual_canon_hash"] = str(visual_canon.get("canon_hash", ""))
    map_visual["agent_sprite_size"] = 64
    ...
    for index, spec_group in enumerate(role_definitions):
        ...
        if not role_inventory:
            raise ValueError(f"Role group {role_name} missing generated inventory; base-role inventory fallback is forbidden.")
        ...
        if not property_library or not knowledge_assets:
            raise ValueError(f"Role group {role_name} missing generated property/knowledge assets; registry fallback is forbidden.")
    ...
    main_character_id_by_name = {
        str(character.get("display_name", "")).strip(): _first_non_empty(
            character.get("agent_id"),
            default=f"{world_slug}_main_{index + 1:02d}",
        )[:128]
    ...
    for index, character in enumerate(specialist_main_characters):
        ...
        main_inventory: list[dict[str, Any]] = []
        for item in character.get("inventory", []):
            if not isinstance(item, dict) or not (item.get("name") or item.get("item_id")):
                continue
            item_id = _slug(str(item.get("item_id") or item.get("name", "")))
            main_inventory.append(
                {
                    "item_id": item_id,
                    ...
                    "metadata": {
                        "name": str(item.get("name", item.get("item_id", ""))),
                        "description": str(item.get("description", "")).strip(),
                        "main_character_generated_inventory": True,
                    },
                }
            )
        if len([item for item in main_inventory if item.get("description")]) < 8:
            raise ValueError(
                f"Main character {character.get('display_name', index + 1)} has insufficient generated inventory; "
                "fallback starting_item_ids are forbidden."
            )
        ...
        if len(property_templates) < 2 or len(knowledge_templates) < 2:
            raise ValueError(
                f"Main character {character.get('display_name', index + 1)} is missing generated property/knowledge templates; "
                "registry fallbacks are forbidden."
            )
        home_room_id = _choose_room_id(
            updated_rooms,
        ...
        for relationship in character.get("relationships", []):
            ...
            target_agent_id = str(main_character_id_by_name.get(target_name, "")).strip()
        ...
        generated_main_characters.append(
            {
                **base_character,
                ...
                "property_library": property_templates,
                "knowledge_assets": knowledge_templates,
    ...
    pure_boutique_cast = (
        bool(generated_main_characters)
        and total_regular_agents == 0
        and int(runtime.get("agent_count", 0) or 0) == len(generated_main_characters)
    )
    if pure_boutique_cast:
        agent_generation["profile_generation"] = {
            "enabled": False,
            "reason": "Boutique character node already generated complete strict profiles.",
        }
        config.setdefault("inventory_generation", {})["enabled"] = False
        config["inventory_generation"]["reason"] = (
            "Boutique character node already generated at least eight validated inventory items per agent."
        )
    ...
    custom_action_rules = world_rules.setdefault("custom_action_rules", {})
    custom_action_rules["default_duration_steps"] = 2
    custom_action_rules["open_proposals_enabled"] = True
    custom_action_rules["open_proposal_effects"] = [
        "speech",
        "transfer",
        "create_item",
        "status",
    ]
    custom_action_rules["world_specific_examples"] = [
        dict(item)
        for item in builder_spec.get("open_action_examples", [])
    ...
    actions["interaction_affordance_catalog"] = _interaction_affordance_catalog(
        actions["ordinary_routes"] + actions["cinematic_routes"]
    )
    catalog_custom_actions = [
        str(item.get("label", "")).strip()
        for item in actions["interaction_affordance_catalog"]
        if str(item.get("kind", "")) in {"custom", "cinematic"}
        and str(item.get("label", "")).strip()
    ]
    actions["allowed_custom_actions"] = _dedupe_texts(
        actions["allowed_custom_actions"] + catalog_custom_actions,
        limit=40,
    )
    actions["routing_policy"] = {
        "ordinary_first": True,
        "open_proposal_route_id": "__propose__",
        "open_proposal_policy": (
            "When no authored route expresses the intended action, propose a bounded world-specific "
            "speech, transfer, object creation, or status effect for World Coordinator review."
        ),
        "world_memory_use": (
    ...
    actions["open_action_examples"] = [
        dict(item) for item in custom_action_rules["world_specific_examples"]
    ]
    ...
    world_progress["world_dynamics"] = dict(builder_spec.get("world_dynamics", {}))
    ...
    image_generation["backend"] = "flux_local_service"
    image_generation["endpoint"] = str(
        config.get("pixel_asset_pipeline", {}).get("sprite_generation", {}).get("endpoint", "http://127.0.0.1:8135")
    )
    image_generation["generate_character_portraits"] = True
    image_generation["response_modalities"] = ["TEXT"]
    image_generation.pop("endpoint_base", None)
    image_generation.pop("method", None)
    image_generation.pop("vertex_cred_file", None)
    image_generation.pop("vertex_project_id", None)
    image_generation.pop("vertex_location", None)
    image_generation["item_image_mode"] = "important_only"
    image_generation["conditions"] = [
        "Still-image routes use Gemini text prompting when needed, then render through the FLUX local service.",
    ]
