...
class PromptBundleSpec(BaseModel):
    ...
    @classmethod
    def _validate_gender_presentation(cls, value: str) -> str:
        normalized = str(value).strip().lower()
        if normalized not in {"feminine", "masculine", "androgynous"}:
            raise ValueError("gender_presentation must be feminine, masculine, or androgynous")
        return normalized
...
class WorldBuilderArtStatusSpec(BaseModel):
    ...
    startup_validation: dict[str, Any] = Field(default_factory=dict)
    map_vision_qa: dict[str, Any] = Field(default_factory=dict)
    map_vision_qa_path: str = ""
    map_vision_qa_history_path: str = ""
