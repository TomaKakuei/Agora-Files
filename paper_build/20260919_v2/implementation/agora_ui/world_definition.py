...
def extract_structured_world_definition(
    ...
    *,
    ...
    existing_matches_scenario = bool(
        existing_structured
        and (
            not scenario_world_id
            or not existing_world_id
            or existing_world_id == scenario_world_id
        )
    )
    bootstrap_structured = (
        {}
        if existing_matches_scenario
        else bootstrap_registered_world_definition(scenario_world_id, scenario_world_name)
    )
    ...
    world_definition = {
        ...
        **dict(existing),
        ...
        "gameplay_loops": [dict(item) for item in existing.get("gameplay_loops", bootstrap_meta.get("gameplay_loops", [])) if isinstance(item, dict)],
        "social_rules": [str(item) for item in existing.get("social_rules", bootstrap_meta.get("social_rules", [])) if str(item).strip()],
        "player_entry_points": [str(item) for item in existing.get("player_entry_points", bootstrap_meta.get("player_entry_points", [])) if str(item).strip()],
        "conflict_hooks": [str(item) for item in existing.get("conflict_hooks", bootstrap_meta.get("conflict_hooks", [])) if str(item).strip()],
        "custom_actions": [str(item) for item in existing.get("custom_actions", bootstrap_meta.get("custom_actions", [])) if str(item).strip()],
        "item_themes": [str(item) for item in existing.get("item_themes", bootstrap_meta.get("item_themes", [])) if str(item).strip()],
        **wallet_config,
    }
...
def sync_world_definition_into_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(pixel_kits, dict):
        ...
        if isinstance(component_library, dict) and component_library:
            map_generation = normalized.setdefault("pixel_asset_pipeline", {}).setdefault("map_generation", {})
            dynamic_library = (
                dict(map_generation.get("component_library", {}))
                if isinstance(map_generation.get("component_library", {}), dict)
                else {}
            )
            map_generation["component_library"] = _deep_merge(component_library, dynamic_library)
