...
def _clean_knowledge_asset(item: dict) -> dict[str, Any]:
    ...
    cleaned["confidence"] = max(0, min(100, int(_safe_int(item.get("confidence"), 80))))
    cleaned["metadata"] = dict(item.get("metadata", {}))
    return cleaned
...
def _room_spawn_cells(room: dict[str, Any]) -> list[GridPosition]:
    ...
    explicit_spawns = [
        GridPosition.model_validate(item)
        for item in room.get("spawn_points", [])
        if isinstance(item, dict)
    ]
    explicit_candidates = [
        cell
        for cell in explicit_spawns
        if (cell.x, cell.y, cell.z) not in obstacles
    ]
    if explicit_candidates:
        return explicit_candidates

    candidate_keys = {(cell.x, cell.y, cell.z) for cell in candidates}
    interior = [
        cell
        for cell in candidates
        if all(
            (cell.x + dx, cell.y + dy, cell.z) in candidate_keys
            for dx, dy in ((-1, 0), (1, 0), (0, -1), (0, 1))
        )
    ]
    return interior or candidates or cells
...
def _inventory_selectable_catalog(catalog: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
    return {
        item_id: item
        for item_id, item in catalog.items()
        if not bool(dict(item.get("metadata", {})).get("forbidden_for_generated_inventory", False))
        and not bool(dict(item.get("metadata", {})).get("frontend_affordance_support_only", False))
    }
...
def _main_character_payload(config: dict[str, Any], spec: dict[str, Any], index_zero: int) -> dict[str, Any]:
    ...
    non_currency_items = [
        ...
        if isinstance(item, dict)
        and str(item.get("item_id", "")) != str(config.get("economy", {}).get("currency_item_id", "currency") or "currency")
    ]
    if len(non_currency_items) < 8:
        raise ValueError(f"main character {display_name} has {len(non_currency_items)} inventory items; generated inventory fallback is forbidden")
    return {
        "agent_id": agent_id,
...
def _build_agent_payloads(config: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    catalog = _inventory_selectable_catalog(_catalog_by_id(config))
...
def _vertex_agent_profile_payloads(
    ...
    *,
    ...
    for base in base_payloads:
        ...
        stage = "main_character_profile_generation" if is_main_character else "agent_profile_generation"
        extra_instruction = ""
...
def _vertex_initial_inventory_payloads(
    ...
    *,
    ...
    if not bool(inv_config.get("enabled", False)):
        return payloads
    catalog = _catalog_by_id(config)
    selectable_catalog = _inventory_selectable_catalog(catalog)
    allowed_item_ids = [
        str(item_id)
        for item_id in inv_config.get("allowed_item_ids", list(catalog.keys()))
        if str(item_id) in selectable_catalog
    ]
    ...
    for index, base in enumerate(payloads, start=1):
        ...
        prompt = (
            ...
            "Choose 8 to 12 distinct carried or operational items that the agent can actually use. "
            "When the agent operates a stall or service, represent bulk stock through sensible quantities of those catalog items and describe the business asset in property_library; do not inflate the number of personal items from role-name keywords.\n"
        ...
        try:
            ...
            raise RuntimeError(f"initial inventory generation failed for agent={base.get('agent_id')}: {exc}") from exc
        ...
        for raw in generated.get("inventory_items", []) if isinstance(generated.get("inventory_items", []), list) else []:
            ...
            if item_id not in selectable_catalog or item_id not in allowed_item_ids:
                continue
            ...
            if len(new_items) >= max_items:
                break
        if not new_items:
            raise ValueError(f"initial inventory generation returned 0 selectable items for agent={base.get('agent_id')}")
        payload = dict(base)
