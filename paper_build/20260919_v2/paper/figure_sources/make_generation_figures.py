...
def _box(
    ...
    *,
    ...
    checks.append((patch, (title_artist, detail_artist), title.replace("\n", " ")))
    ax._agora_box_checks = checks


def _validate_box_text(fig: plt.Figure, ax: plt.Axes) -> None:
    """Fail figure generation when a box label escapes its visible boundary."""
    fig.canvas.draw()
    renderer = fig.canvas.get_renderer()
    failures: list[str] = []
    for patch, artists, label in getattr(ax, "_agora_box_checks", []):
        boundary = patch.get_window_extent(renderer).expanded(0.96, 0.91)
        artist_bounds = [artist.get_window_extent(renderer) for artist in artists]
        for artist in artists:
            ...
            if not (
                ...
                failures.append(
                    f"{label} [{artist.get_text().replace(chr(10), ' / ')}] "
                    f"text=({bounds.x0:.0f},{bounds.y0:.0f},{bounds.x1:.0f},{bounds.y1:.0f}) "
                    f"box=({boundary.x0:.0f},{boundary.y0:.0f},{boundary.x1:.0f},{boundary.y1:.0f})"
                )
                break
        if artist_bounds[0].overlaps(artist_bounds[1]):
            failures.append(f"{label} [title overlaps detail]")
    if failures:
...
def architecture_figure(output_root: Path) -> None:
    ...
    ax.text(
        5.35,
        0.28,
        "local refinement updates one room, object, or character while preserving world identity",
        fontsize=7.2,
        color=RED,
        ha="center",
    )
    _validate_box_text(fig, ax)
