...
criteria. We additionally release matched English and Chinese artifact-only
questionnaires for human evaluation of the generated worlds without runtime
access. Matched two-round language-model runs across those worlds produce 53
...
\subsection{Document-Based Human World Evaluation}

Automated completeness and consistency checks are complemented by a
self-contained human evaluation protocol. Each rater receives the original
one-sentence premise and an equal-size evidence set for every world: one map,
one representative character sheet, rules, activity loops, places, people and
motives, objects, and possible player actions. Model identities, runtime traces,
and system scores are omitted. The judgment therefore concerns the generated
world artifact itself rather than execution reliability or observed agent
behavior.

The bilingual English and Chinese forms use the same seven anchored five-point
items. Six primary dimensions measure premise realization, systemic coherence,
society and character, spatial and material specificity, meaningful interaction
possibilities, and visual coherence. For rater $r$ and world $w$, the primary
score is
\[
H_{r,w}=25\left(\frac{1}{6}\sum_{d=1}^{6}q_{r,w,d}-1\right),
\]
which maps their equal-weight mean to $[0,100]$. A seventh item records overall
world quality separately. Three short responses ask what consequential action
the rater would try first, which element makes the world distinctive, and which
single element most needs improvement. Assigned starting worlds are balanced
across A--E to vary presentation position without replacing the absolute scale
...
\centering
\footnotesize
\begin{tabularx}{\columnwidth}{@{}X>{\centering\arraybackslash}p{0.26\columnwidth}>{\centering\arraybackslash}p{0.18\columnwidth}@{}}
\toprule
...
\footnotesize
\setlength{\tabcolsep}{2pt}
\begin{tabularx}{\columnwidth}{@{}>{\raggedright\arraybackslash}p{0.19\columnwidth}>{\raggedright\arraybackslash}p{0.43\columnwidth}>{\raggedright\arraybackslash}X@{}}
\toprule
Claim & Evidence here & Follow-up \\
\midrule
One sentence & 10 paired premises & broader domains \\
World quality & 5 worlds; 58/58 defects; bilingual form & human ratings \\
Interaction breadth & 11--12 affordances per world & participant tasks \\
Open intention & 10 approved proposals & proposal ablation \\
Social process & 53 fresh; 1,052 pilot events & matched long runs \\
Human co-play & 13 targeted events & controlled study \\
\bottomrule
\end{tabularx}
