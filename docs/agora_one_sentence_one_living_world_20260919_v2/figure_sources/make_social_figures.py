...
def box(ax, x, y, w, h, title, body, color):
    ...
    title_artist = ax.text(
        x + 0.018, y + h - 0.035, title, fontsize=8.7,
        fontweight="bold", va="top", linespacing=1.0,
    )
    title_lines = title.count("\n") + 1
    body_artist = ax.text(
        x + 0.018, y + h - 0.105 - 0.052 * (title_lines - 1), body,
        fontsize=7.0, va="top", color=COLORS["ink"], linespacing=1.20,
    )
    checks = getattr(ax, "_agora_box_checks", [])
    checks.append((patch, (title_artist, body_artist), title.replace("\n", " ")))
    ax._agora_box_checks = checks
    return patch


def validate_box_text(fig, ax):
    """Fail figure generation when text is not fully contained by its box."""
    fig.canvas.draw()
    renderer = fig.canvas.get_renderer()
    failures = []
    for patch, artists, label in getattr(ax, "_agora_box_checks", []):
        boundary = patch.get_window_extent(renderer).expanded(0.95, 0.90)
        for artist in artists:
            bounds = artist.get_window_extent(renderer)
            if not (
                bounds.x0 >= boundary.x0
                and bounds.x1 <= boundary.x1
                and bounds.y0 >= boundary.y0
                and bounds.y1 <= boundary.y1
            ):
                failures.append(label)
                break
    if failures:
        raise RuntimeError(f"text outside diagram boxes: {', '.join(failures)}")
...
def coordinator_loop():
    ...
    nodes = [
        ...
        (0.22, "2. Form\nintention", "choose a known action\nor propose a new,\ntarget-specific action", COLORS["gold"]),
        (0.42, "3. Coordinate", "check location, ownership,\nconsent, rules,\nand consequences", COLORS["coral"]),
        (0.62, "4. Change\nworld", "update inventory, objects,\nrelationships, location,\nor status", COLORS["green"]),
        (0.82, "5. Remember", "record result or failure\nupdate episodic memory\nand next-round context", COLORS["teal"]),
    ]
    ...
    ax.add_patch(FancyArrowPatch((0.90, 0.30), (0.10, 0.30), connectionstyle="arc3,rad=-0.10",
                                 arrowstyle="-|>", mutation_scale=12, color=COLORS["gray"], linewidth=1.3))
    ax.text(0.50, 0.075, "the next decision observes the consequences of the previous action",
            ha="center", fontsize=8.5, color=COLORS["ink"])
    validate_box_text(fig, ax)
...
def clockwork_episode():
    ...
    ax.set_xlim(0, 1); ax.set_ylim(0, 1); ax.axis("off")
    stages = [
        (0.02, "World event", "Weather-sheet press\njams; production halts.", COLORS["gray"]),
...
def experiment_design():
    ...
    ax.set_xlim(0, 1); ax.set_ylim(0, 1); ax.axis("off")
    box(ax, 0.02, 0.35, 0.17, 0.42, "Seeded fact", "Round 0: three lower-\narchive agents privately\nlearn of a flood threat.", COLORS["gold"])
    box(ax, 0.27, 0.58, 0.22, 0.30, "Full memory", "episodic memory\n+ task threads\n+ relationships + world state", COLORS["teal"])
    box(ax, 0.27, 0.16, 0.22, 0.30, "Memory ablation", "same profiles, model, seed,\nand world; remove prior episodes", COLORS["coral"])
    box(ax, 0.58, 0.35, 0.17, 0.42, "32 rounds", "transmit the warning\ncross rooms, recruit roles\nfind objects, move treaty", COLORS["green"])
    box(ax, 0.82, 0.35, 0.16, 0.42, "Measured\noutcomes", "diffusion reach\ncross-room transfer time\nrescue success\nnetwork / norm violations", COLORS["teal"])
