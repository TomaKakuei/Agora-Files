\section{Persistent State and Execution Evidence}
\label{sec:extended}
...
Supplementary Table~\ref{supp-tab:extended-audit} retains every failure. These are recorded API
...
Supplementary Section~\ref{supp-app:extended} provides the full audit and replication boundaries.

\subsection{From a Proposal to a Later Observation}
A retained Clockwork trajectory makes the state-transition mechanism concrete.
At round 11, Thistle Thorne proposes giving one Corrosive Inkwell to Elara
Moss. The structured effect names the existing item, source, recipient, and
quantity. The coordinator checks ownership, co-location, and effect bounds,
then records a transfer. In the next planning observation at round 13, the
item appears in Elara's inventory and is absent from Thistle's. A separate
scripted attempt to transfer an unowned item is rejected with identical
before/after state hashes. This traces the connection from a generated
possession through a proposed action to a persistent consequence and a later
observation; the transfer alone does not establish completion of its stated
goal. Supplementary Section~\ref{supp-app:ablation} identifies the source records
and reports all 18 planner-diagnostic trajectories, including the full
input-history and action-space ablation tables.
