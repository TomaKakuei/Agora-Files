...
\item An auditable account of how these mechanisms operate: seeded defects
and local repairs test construction dependencies; interaction records expose
action selection, coordinator decisions, and committed state changes. Existing
visual judgments characterize the rendered artifacts, and supplementary
planner diagnostics document input and action-space restrictions.
\end{enumerate}

The paper asks how a premise-generated setting becomes an environment with
executable consequences. Its evidence follows the mechanisms from generation
contracts through action adjudication to persistent state. Model-specific
artifact judgments and supplementary configuration diagnostics provide
implementation context; behavioral fidelity and causal social effects remain
outside the completed studies' scope.
...
and the present human artifact judgments separate.
...
these reporting priorities in Supplementary Section~\ref{supp-app:repro}. Human-grounded agent
