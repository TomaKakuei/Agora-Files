...
class CoreAIMixin:
    ...
    def _live_ai_config(self) -> dict[str, Any]:
        ...
        live_max_output_tokens = _safe_int(
            os.environ.get("AGORA_LIVE_AI_MAX_OUTPUT_TOKENS", vertex_api.get("live_max_output_tokens", 1024)),
            1024,
        )
        live_thinking_level = str(
            os.environ.get("AGORA_LIVE_AI_THINKING_LEVEL", vertex_api.get("live_thinking_level", "low"))
        ).strip() or "low"
        live_thinking_budget = _safe_int(
            os.environ.get("AGORA_LIVE_AI_THINKING_BUDGET", vertex_api.get("live_thinking_budget", 128)),
            128,
        )
        vertex_api.update(
            {
