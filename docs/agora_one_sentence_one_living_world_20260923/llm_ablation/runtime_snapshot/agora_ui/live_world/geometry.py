...
def _room_traversable_tiles(room: dict[str, Any]) -> list[dict[str, int]]:
    obstacle_keys = {
        _room_tile_key(
            _safe_int(item.get("x", 0)),
            _safe_int(item.get("y", 0)),
            _safe_int(item.get("z", 0)),
        )
    ...
    return [
        tile
        for tile in _room_tiles(room)
        if _room_tile_key(tile["x"], tile["y"], tile["z"]) not in obstacle_keys
    ]


def _room_interior_tiles(room: dict[str, Any]) -> list[dict[str, int]]:
    tiles = _room_traversable_tiles(room)
    tile_keys = {_room_tile_key(tile["x"], tile["y"], tile["z"]) for tile in tiles}
    return [
        tile
        for tile in tiles
        if all(
            _room_tile_key(tile["x"] + dx, tile["y"] + dy, tile["z"]) in tile_keys
            for dx, dy in ((-1, 0), (1, 0), (0, -1), (0, 1))
        )
    ]


def _safe_room_spawn(
    room: dict[str, Any],
    agent_id: str,
    requested_position: dict[str, Any] | None = None,
) -> dict[str, int]:
    traversable = _room_traversable_tiles(room)
    if not traversable:
        return _coord_payload(requested_position)

    interior = _room_interior_tiles(room)
    traversable_keys = {
        _room_tile_key(tile["x"], tile["y"], tile["z"])
    ...
    doorway_keys = _room_door_tile_keys(room) & traversable_keys
    requested = _coord_payload(requested_position)
    requested_key = _room_tile_key(requested["x"], requested["y"], requested["z"])

    if requested_key in traversable_keys:
        mobile_landing_keys = interior_keys | doorway_keys
        has_mobile_neighbor = any(
            _room_tile_key(requested["x"] + dx, requested["y"] + dy, requested["z"]) in mobile_landing_keys
            for dx, dy in ((-1, 0), (1, 0), (0, -1), (0, 1))
        )
        if requested_key in mobile_landing_keys or has_mobile_neighbor or not interior:
            return requested

    candidates = interior or [
        tile
        for tile in traversable
        if _room_tile_key(tile["x"], tile["y"], tile["z"]) in doorway_keys
    ] or traversable
    candidates = sorted(candidates, key=lambda item: (item["y"], item["x"], item["z"]))
    offset = int(
        hashlib.sha256(f"{room.get('room_id', '')}:{agent_id}:spawn".encode("utf-8")).hexdigest()[:8],
        16,
    ) % len(candidates)
    return dict(candidates[offset])


def _room_walk_path(room: dict[str, Any], agent_id: str) -> list[dict[str, int]]:
    interior = _room_interior_tiles(room)
    tiles = interior if len(interior) >= 2 else _room_traversable_tiles(room)
    if not tiles:
        return []
...
__all__ = ['_room_tile_key', '_coord_payload', '_coord_distance', '_resolve_map_grid_path', '_load_room_list', '_room_bounds', '_room_tiles', '_room_tile_index', '_room_door_tile_keys', '_room_wall_indexes', '_room_traversable_tiles', '_room_interior_tiles', '_safe_room_spawn', '_room_walk_path', '_resolve_room_transition', '_tile_has_wall_kind', '_preferred_room_id_for_tile', '_resolve_wall_hop_destination', '_disable_world_geometry_collision', '_resolve_unblocked_destination']
