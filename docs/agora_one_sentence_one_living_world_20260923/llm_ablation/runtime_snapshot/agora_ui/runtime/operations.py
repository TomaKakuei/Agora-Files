...
from ..world_coordinator import evaluate_open_action, proposal_route
...
def _validate_materialized_agent_ids(scenario_dir: Path, expected_payloads: list[dict[str, Any]]) -> None:
    expected = {str(item.get("agent_id", "")).strip() for item in expected_payloads}
    expected.discard("")
    actual = {path.stem for path in (scenario_dir / "Agents").glob("*.json")}
    if actual != expected:
        unexpected = ", ".join(sorted(actual - expected)[:8]) or "none"
        missing = ", ".join(sorted(expected - actual)[:8]) or "none"
        raise ValueError(
            "materialized scenario agent identity mismatch; "
...
def initialize_runtime(ctx: RuntimeExecutionContext, node: dict[str, Any]) -> None:
    ...
    if args.scenario_dir:
        ...
        # Scenario files are mutable runtime state. Keeping them inside the run
        # prevents concurrent worlds from overwriting one another's Agents/.
...
def materialize_or_resume_state(ctx: RuntimeExecutionContext, node: dict[str, Any]) -> None:
    ...
    if resume_in_place:
        ...
        profile_generation = config.get("agent_generation", {}).get("profile_generation", {})
        profiles_enabled = not isinstance(profile_generation, dict) or bool(profile_generation.get("enabled", True))
        if not profiles_enabled:
            agent_payloads = base_agent_payloads
            agent_profile_source = "compiled_world_profiles"
            legacy.dump_json(
                run_dir / "profile_generation_run.json",
                {
                    ...
                    "reason": str(profile_generation.get("reason", "profile generation disabled")),
                },
            )
        elif reuse_profile_cache_dir is not None:
        ...
        _validate_materialized_agent_ids(scenario_dir, agent_payloads)
...
def request_route(ctx: RuntimeExecutionContext, node: dict[str, Any]) -> None:
    ...
    target = ctx.require("current_target")
    human_cfg = _human_config(ctx)
    human_events = ctx.get("current_human_events", [])
    human_route_id = ""
    human_proposal = None
    if (
        ...
        human_route_id = str(human_events[-1].get("affordance_id", "")).strip()
        if isinstance(human_events[-1].get("open_action_proposal"), dict):
            human_proposal = dict(human_events[-1]["open_action_proposal"])
    route = legacy._route_lookup(ctx.config).get(human_route_id, {}) if human_route_id else {}
    if human_proposal is not None:
        request = {"route_id": "__propose__", "kind": "custom", "proposed_action": human_proposal}
    elif route:
        request = {
            "route_id": human_route_id,
            "kind": str(route.get("kind", "custom")),
            "reason": "explicit human affordance selection",
    ...
    if str(request.get("route_id", "")) == "__propose__":
        decision = evaluate_open_action(
            dict(request.get("proposed_action", {})),
            actor=actor,
            target=target,
            state=ctx.require("state"),
            config=ctx.config,
        ...
        if decision.status == "approved":
            request = {
                **proposal_route(decision),
                ...
                "relationship_adjustments": request.get("relationship_adjustments", []),
            }
        else:
            fallback = legacy._route_lookup(ctx.config).get("creator_world_chat", {})
            request = {
                **fallback,
                "reason": f"World Coordinator {decision.status}: {decision.reason}",
...
def write_final_outputs(ctx: RuntimeExecutionContext, node: dict[str, Any]) -> None:
    ...
    story_payload = {
        ...
        "resumed_from": {
            "source_run_dir": str(ctx.get("resume_run_dir")) if ctx.get("resume_run_dir") is not None else "",
            "completed_round": ctx.get("resume_completed_round", 0),
            "start_round": int(ctx.get("resume_completed_round", 0)) + 1,
        },
