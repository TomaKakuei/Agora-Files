...
def _queue_generation_worker(
    *,
    ...
    inline_mode = str(os.environ.get("AGORA_WORLD_CREATOR_INLINE", "")).strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
    ...
    if inline_mode:
        payload = {
            ...
            "updated_at": _now_iso(),
            "detail": "Generation worker prepared for inline execution.",
            "request_kind": str(request_kind or "create"),
            "draft_id": draft_id,
            "revision_id": revision_id,
...
def create_draft(package_root: Path, request: dict[str, Any]) -> dict[str, Any]:
    ...
    _save_manifest(package_root, draft_id, manifest)
    if (
        str(os.environ.get("AGORA_WORLD_CREATOR_INLINE", "")).strip().lower() in {"1", "true", "yes", "on"}
        and str(generation_payload.get("status", "")) != STATUS_DRAFT_FAILED
    ):
        run_generation_worker(package_root, draft_id, revision_id)
...
def _publish_metadata_from_export(
    package_root: Path,
    access_code: str,
    *,
    draft_id: str,
    ...
    export_dir = package_root / "output" / "package_exports" / normalized
    exported_db = export_dir / "world_package.db"
    if not normalized or not exported_db.is_file():
        raise FileNotFoundError(f"publish export package missing: {normalized}")

    existing_meta_path = export_dir / "package_meta.json"
    if existing_meta_path.is_file():
        existing = _read_json(existing_meta_path)
        if bool(existing.get("startup_ok", False)):
            return existing
    ...
    config = sync_world_definition_into_config(_read_json(config_path))
    scenario_meta = config.get("scenario_meta", {}) if isinstance(config.get("scenario_meta", {}), dict) else {}
    world_name = str(scenario_meta.get("world_name", "")).strip() or package_name or normalized
    world_id = str(scenario_meta.get("world_id", "")).strip()
    runtime = config.get("runtime", {}) if isinstance(config.get("runtime", {}), dict) else {}
    raw_seed = str(runtime.get("seed", 42627)).strip()
    seed = int(raw_seed) if raw_seed.isdigit() else 42627
    ...
    if not bool(backend_startup_validation.get("startup_ok", False)):
        raise RuntimeError(
            "recovered publish export failed live startup smoke test: "
    ...
    pixel_launch_validation = validate_pixel_ui_launch(
        ...
        normalized,
        seed=seed,
        require_ai_reply=False,
    )
    if not bool(pixel_launch_validation.get("startup_ok", False)):
        raise RuntimeError(
            "recovered publish export failed Pixel UI launch validation: "
    ...
    metadata = {
        ...
        "recovered_publish_export": True,
    }
    _write_json(existing_meta_path, metadata)
    return metadata


def _recover_publish_metadata(
    package_root: Path,
    *,
    draft_id: str,
    revision_id: str,
    ...
    exports_root = package_root / "output" / "package_exports"
    if not exports_root.is_dir():
        return None
    candidates: list[tuple[float, str]] = []
    for exported_db in exports_root.glob("*/world_package.db"):
        try:
            metadata = read_world_package_metadata(exported_db)
        except Exception:
            continue
        if str(metadata.get("source_label", "")).strip() != "world_creator_publish":
            continue
        if str(metadata.get("world_creator_draft_id", "")).strip() != draft_id:
            continue
        if str(metadata.get("world_creator_revision", "")).strip() != revision_id:
            continue
        candidates.append((exported_db.stat().st_mtime, exported_db.parent.name))
    for _, access_code in sorted(candidates, reverse=True):
        return _publish_metadata_from_export(
            package_root,
            access_code,
            draft_id=draft_id,
            revision_id=revision_id,
...
def _promote_validated_art_export(
    package_root: Path,
    *,
    draft_id: str,
    revision_id: str,
    revision_package: Path,
    package_name: str,
) -> dict[str, Any] | None:
    art_status_path = _revision_art_status_path(package_root, draft_id, revision_id)
    if not art_status_path.is_file():
        return None
    art_status = _read_json(art_status_path)
    map_qa = art_status.get("map_vision_qa", {}) if isinstance(art_status.get("map_vision_qa", {}), dict) else {}
    startup = art_status.get("startup_validation", {}) if isinstance(art_status.get("startup_validation", {}), dict) else {}
    pixel_launch = art_status.get("pixel_launch_validation", {}) if isinstance(art_status.get("pixel_launch_validation", {}), dict) else {}
    expected_code = str(pixel_launch.get("expected_access_code", "")).strip()
    selected_code = str(pixel_launch.get("selected_access_code", "")).strip()
    validated_source = Path(str(pixel_launch.get("package_db", "")).strip()).expanduser()
    if not (
        str(art_status.get("status", "")).strip() == STATUS_PUBLISH_READY
        and map_qa.get("is_pixel_map") == "Y"
        and map_qa.get("has_visual_errors") == "N"
        and bool(startup.get("startup_ok", False))
        and bool(pixel_launch.get("startup_ok", False))
        and expected_code
        and expected_code == selected_code
        and validated_source.is_absolute()
        and validated_source.resolve() == revision_package.resolve()
    ):
        return None

    export_dir = package_root / "output" / "package_exports" / expected_code
    exported_db = export_dir / "world_package.db"
    if not exported_db.is_file():
        return None
    workspace = ensure_materialized_world_package(exported_db, output_dir=export_dir / "materialized")
    pixel_report = assess_pixel_readiness_from_root(workspace)
    ...
    config = sync_world_definition_into_config(_read_json(config_path))
    scenario_meta = config.get("scenario_meta", {}) if isinstance(config.get("scenario_meta", {}), dict) else {}
    world_name = str(scenario_meta.get("world_name", "")).strip() or package_name or expected_code
    world_id = str(scenario_meta.get("world_id", "")).strip()
    backend_startup = validate_world_package_startup(
        package_root,
        expected_code,
        display_name=world_name,
    )
    if not bool(backend_startup.get("startup_ok", False)):
        return None
    ...
    promoted_startup = {
        **startup,
        "startup_ok": True,
        "stage": "ok",
        "expected_access_code": expected_code,
        "selected_access_code": selected_code,
        "backend_startup_validation": backend_startup,
        "pixel_launch_validation": pixel_launch,
        "error": "",
    }
    metadata = {
        ...
        "pixel_read_report": json.dumps(pixel_report, ensure_ascii=False),
        "startup_ok": True,
        "package_validation": {"stage": "ok", "pixel_read": True},
        "backend_startup_validation": backend_startup,
        "pixel_launch_validation": pixel_launch,
        "startup_validation": promoted_startup,
        "world_creator_draft_id": draft_id,
        "world_creator_revision": revision_id,
        "promoted_art_validation": True,
    }
    _write_json(export_dir / "package_meta.json", metadata)

    # A failed publish attempt may have copied the same revision before this
    # promotion path was available. Keep those incomplete exports hidden.
    for candidate_db in (package_root / "output" / "package_exports").glob("*/world_package.db"):
        if candidate_db == exported_db:
            continue
        try:
            candidate_meta = read_world_package_metadata(candidate_db)
        except Exception:
            continue
        if (
            str(candidate_meta.get("source_label", "")).strip() != "world_creator_publish"
            or str(candidate_meta.get("world_creator_draft_id", "")).strip() != draft_id
            or str(candidate_meta.get("world_creator_revision", "")).strip() != revision_id
        ):
            continue
        package_meta_path = candidate_db.parent / "package_meta.json"
        package_meta = _read_json(package_meta_path) if package_meta_path.is_file() else {}
        if bool(package_meta.get("startup_ok", False)):
            continue
        with sqlite3.connect(candidate_db) as conn:
            conn.execute(
                """
                INSERT INTO meta(key, value) VALUES('validation_probe', '1')
                ON CONFLICT(key) DO UPDATE SET value = excluded.value
                """
            )
            conn.commit()
...
def publish_draft(package_root: Path, draft_id: str) -> dict[str, Any]:
    ...
    metadata = _promote_validated_art_export(
        package_root,
        draft_id=draft_id,
        revision_id=revision_id,
    ...
    if metadata is None:
        metadata = _recover_publish_metadata(
            package_root,
            draft_id=draft_id,
            revision_id=revision_id,
            package_name=package_name,
