...
def _pixel_validation_timeout_seconds() -> float:
    return max(60.0, _env_float("AGORA_PIXEL_VALIDATION_TIMEOUT_SECONDS", 600.0))
...
def validate_pixel_ui_launch(
    ...
    *,
    ...
    effective_timeout_seconds = (
        _pixel_validation_timeout_seconds()
        if timeout_seconds is None
        else max(60.0, float(timeout_seconds))
    )
    result_timeout_seconds = max(
        60.0,
        _env_float(
            "AGORA_PIXEL_RESULT_TIMEOUT_SECONDS",
            max(300.0, effective_timeout_seconds - 120.0),
        ),
    )
    snapshot_timeout_seconds = max(
        30.0,
        _env_float("AGORA_PIXEL_SNAPSHOT_TIMEOUT_SECONDS", 120.0),
    )
    command = [
        py_bin,
        str(script_path),
        ...
        str(snapshot_timeout_seconds),
    ]
    if not require_ai_reply:
        command.append("--skip-ai-reply")
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=max(effective_timeout_seconds, result_timeout_seconds + snapshot_timeout_seconds + 60.0),
            env=env,
            check=False,
        )
    ...
    report["validation_profile"] = str(
        payload.get("validation_profile")
        or report["validation_profile"]
    ).strip()
    ai_persistence = payload.get("ai_persistence", {})
    if isinstance(ai_persistence, dict):
        report["ai_reply_checked"] = bool(ai_persistence.get("ai_reply_checked", require_ai_reply))
