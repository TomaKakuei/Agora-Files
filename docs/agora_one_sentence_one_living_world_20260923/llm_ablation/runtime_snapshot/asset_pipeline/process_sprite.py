...
def _quantize_frame(
    ...
    *,
    ...
    # FLUX often paints a pale floor ellipse or reflection that touches the
    # boots and therefore survives connected-component cleanup. Restrict this
    # cleanup to the bottom foot band so light clothing remains untouched.
    pixels = np.array(quantized, dtype=np.uint8)
    foot_band_start = int(pixels.shape[0] * 0.88)
    foot_band = pixels[foot_band_start:, :, :]
    rgb = foot_band[:, :, :3]
    low_saturation = (rgb.max(axis=2) - rgb.min(axis=2)) < 45
    bright = rgb.mean(axis=2) > 160
    foot_band[:, :, 3][low_saturation & bright] = 0
    quantized = Image.fromarray(pixels, mode="RGBA")
    return quantized
...
def _alignment_policy(policy: dict[str, Any] | None, target_frame_height: int) -> dict[str, Any]:
    ...
    merged["min_body_width_ratio"] = float(merged.get("min_body_width_ratio", 0.28))
...
def _state_quality_report(
    ...
    *,
    ...
    frame_height = max(1, int(target_frame_height))
    frame_width = max(1, int(target_frame_width))
    ...
    readable_width_ratio = float(policy["min_body_width_ratio"])
    if state.name in {"walk_left", "walk_right"}:
        readable_width_ratio *= 0.6
    if state.static_frame_index is not None:
        pass_state = (
            non_empty_frames >= 1
            ...
            and max_width_ratio >= readable_width_ratio
...
def build_phaser_atlas(
    *,
    ...
    for state in states:
        ...
        state_quality = _state_quality_report(
            state,
            state_metrics,
            policy,
            target_frame_width=target_frame_width,
