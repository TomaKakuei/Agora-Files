...
def directional_sheet_qa(image_path: str | Path) -> dict[str, Any]:
    """Verify the 4x4 runtime direction contract without a vision model."""
    path = Path(image_path)
    if not path.is_file():
        return {"pass": False, "reason": "raw sheet missing"}
    sheet = Image.open(path).convert("RGBA")
    if sheet.width % 4 or sheet.height % 4:
        return {"pass": False, "reason": "raw sheet is not divisible into a 4x4 grid"}

    cell_width = sheet.width // 4
    cell_height = sheet.height // 4
    mirror_mismatches: list[float] = []
    side_asymmetries: list[float] = []
    front_profile_differences: list[float] = []
    for column in range(4):
        bounds = (column * cell_width, (column + 1) * cell_width)
        walk_down = sheet.crop((bounds[0], cell_height, bounds[1], 2 * cell_height))
        walk_left = sheet.crop((bounds[0], 2 * cell_height, bounds[1], 3 * cell_height))
        walk_right = sheet.crop((bounds[0], 3 * cell_height, bounds[1], 4 * cell_height))
        mirrored_left = ImageOps.mirror(walk_left)
        pixel_count = max(1, cell_width * cell_height)

        def changed_ratio(first: Image.Image, second: Image.Image) -> float:
            return sum(
                1
                for expected, actual in zip(first.getdata(), second.getdata())
                if max(abs(expected[index] - actual[index]) for index in range(4)) > 8
            ) / pixel_count

        mirror_mismatches.append(changed_ratio(mirrored_left, walk_right))
        side_asymmetries.append(changed_ratio(walk_left, mirrored_left))
        front_profile_differences.append(changed_ratio(walk_down, walk_left))

    maximum_mismatch = max(mirror_mismatches, default=1.0)
    minimum_asymmetry = min(side_asymmetries, default=0.0)
    minimum_front_profile_difference = min(front_profile_differences, default=0.0)
    return {
        "pass": (
            maximum_mismatch <= 0.01
            and minimum_asymmetry >= 0.08
            and minimum_front_profile_difference >= 0.12
        ),
        "maximum_mirror_mismatch_ratio": round(maximum_mismatch, 4),
        "minimum_left_row_asymmetry_ratio": round(minimum_asymmetry, 4),
        "minimum_front_profile_difference_ratio": round(minimum_front_profile_difference, 4),
    }
...
def _frame_edge_connected_near_white_pixels(
    ...
    *,
    ...
    def is_near_white(index: int) -> bool:
        red, green, blue, alpha = pixels[index]
        return (
            alpha >= alpha_threshold
            and red >= near_white_threshold
            and green >= near_white_threshold
            and blue >= near_white_threshold
            and (max(red, green, blue) - min(red, green, blue)) <= neutral_tolerance
        )

    connected: set[int] = set()
    for frame_top in range(0, height, frame_height):
        frame_bottom = min(height, frame_top + frame_height)
        for frame_left in range(0, width, frame_width):
            frame_right = min(width, frame_left + frame_width)
            queue: deque[tuple[int, int]] = deque()
            for x in range(frame_left, frame_right):
                queue.append((x, frame_top))
                queue.append((x, frame_bottom - 1))
            for y in range(frame_top, frame_bottom):
                queue.append((frame_left, y))
                queue.append((frame_right - 1, y))
            while queue:
                x, y = queue.popleft()
                if x < frame_left or x >= frame_right or y < frame_top or y >= frame_bottom:
                    continue
                index = y * width + x
                if index in connected or not is_near_white(index):
                    continue
                connected.add(index)
                queue.extend(
                    (
...
def final_atlas_transparency_qa(
    *,
    ...
    frame_width = max(1, int(processing_config["target_frame_width"]))
    frame_height = max(1, int(processing_config["target_frame_height"]))
    ...
    edge_connected_near_white_pixels = _frame_edge_connected_near_white_pixels(
        pixels,
        width=width,
        height=height,
    ...
    edge_connected_near_white_ratio = edge_connected_near_white_pixels / float(total_pixels)
    ...
    if edge_connected_near_white_ratio > max_near_white_opaque_ratio:
        failures.append(
            "Atlas frame-edge-connected near-white ratio "
            f"{edge_connected_near_white_ratio:.3f} exceeds "
            f"{max_near_white_opaque_ratio:.3f}."
        )
    ...
    return {
        ...
        "frame_edge_connected_near_white_ratio": float(
            round(edge_connected_near_white_ratio, 4)
        ),
...
def strict_programmatic_qa(
    *,
    ...
    min_body_width_ratio = float(policy.get("min_body_width_ratio", 0.28))
    ...
    for state in states:
        ...
        for frame_index in range(state.frame_count):
            ...
            if bbox is None:
                ...
                disconnected = (
                    ...
                    or float(component_summary["largest_component_ratio"]) < min_largest_component_ratio
                )
                if disconnected:
        ...
        readable_width_ratio = min_body_width_ratio * (0.6 if state.name in {"walk_left", "walk_right"} else 1.0)
        state_pass = (
            ...
            and max_width_ratio_state >= readable_width_ratio
        ...
        if not state_pass:
            ...
            if max_width_ratio_state < readable_width_ratio:
                reasons.append(f"readable body width ratio {max_width_ratio_state:.3f} < {readable_width_ratio:.3f}")
        ...
        state_reports.append(
            {
                ...
                "min_readable_body_width_ratio": float(readable_width_ratio),
