...
def _apply_vision_false_negative_override(
    *,
    ...
    if effective.get("status") != "ok" or bool(effective.get("pass_qa")):
        return effective
    if not bool(programmatic_qa.get("pass_qa")):
        return effective
    ...
    if not bool(effective.get("background_pass")):
        black_background_false_negative = _looks_like_black_background_false_negative(
            effective.get("background_check", ""),
            effective.get("final_suggestion", ""),
        )
        minor_edge_noise = _looks_like_minor_edge_noise_consistency_false_negative(
            effective.get("background_check", ""),
            effective.get("consistency_check", ""),
            effective.get("final_suggestion", ""),
        )
        if not black_background_false_negative and not minor_edge_noise:
            return effective
        effective["background_pass"] = True
        override_reasons.append(
            "Programmatic transparency evidence resolves a specific checkerboard/edge-noise false negative."
        )

    if not bool(effective.get("consistency_pass")):
        optional_accessory = _looks_like_optional_accessory_consistency_false_negative(
            effective.get("consistency_check", ""),
            effective.get("final_suggestion", ""),
        )
        minor_edge_noise = _looks_like_minor_edge_noise_consistency_false_negative(
        ...
        if not optional_accessory and not minor_edge_noise:
            return effective
        effective["consistency_pass"] = True
        override_reasons.append(
            "Programmatic geometry evidence resolves a specific accessory/edge-noise false negative."
        )

    effective["effective_pass_qa"] = all(
        bool(effective.get(key))
        for key in ("anatomy_pass", "animation_pass", "consistency_pass", "background_pass")
    )
    if effective["effective_pass_qa"] and override_reasons:
        effective["override_reason"] = " ".join(override_reasons)
    return effective
