...
def _generated_sprite_provenance_passes(revision_dir: Path) -> tuple[bool, dict[str, Any]]:
    ...
    reused_mode = str(
        reused_raw_summary.get("mode") or sprite_summary.get("mode") or ""
    ).strip().lower()
    is_reused = (
        reused_status == "reused_latest_raw_sheet"
        or reused_mode == "reused_latest_raw_sheet"
    )
    ...
    allowed_sprite_statuses = {
        "ok",
        "normalized_remote_sheet",
    ...
    allowed_reused_statuses = {"", "not_used", "reused_latest_raw_sheet"}
    legacy_explicit_reuse = reused_status == "ok" and is_reused
    if reused_status not in allowed_reused_statuses and not legacy_explicit_reuse:
...
def validate_sprite_integrity(
    ...
    *,
    ...
    failures: list[str] = []
    if edge_bleed:
        failures.append("Opaque pixels touch the frame boundary.")
...
def _validate_atlas_frames(
    ...
    *,
    ...
    target_frame_width = int(processing.get("target_frame_width", 64))
    target_frame_height = int(processing.get("target_frame_height", 64))
...
def _best_existing_ready_event(
    ...
    expected_world_id: str = "",
    expected_world_revision: str = "",
    allow_foreign_revision_fallback: bool = False,
    ...
    for revision_dir in ordered_revision_dirs:
        ...
        if (
            ...
            or not raw_sheet_pass
        ):
            print(
                f"[REJECT] Agent {agent_id} in dir {revision_dir} skipped: "
                f"event={bool(event)}, atlas={atlas_path.is_file()}, "
                f"prov_match={provenance_match}, prov_pass={provenance_pass}, "
                f"raw_sheet_pass={raw_sheet_pass}"
            )
            continue
        ...
        integrity_pass = bool(passed and integrity_report["pass"])
        ...
        if integrity_pass:
            ...
            print(
                f"[REJECT] Agent {agent_id} in dir {revision_dir} skipped: "
                f"passed={passed}, integrity_pass={integrity_pass}"
            )
    return None, None
...
def _write_current_aliases(repo_root: Path, manifest_payload: dict[str, Any]) -> None:
    ...
    alias_paths = [
        ...
        generated_root / "guild_asset_sets" / "current_guild_pixel_set.json",
    ]
...
def _canonicalize_revision_manifest(
    ...
    *,
    ...
    manifest = _read_json(manifest_path) if manifest_path.is_file() else {
        ...
        "agents": [{"agent_id": str(entry.get("id", "")).strip()} for entry in assets],
        "map_generation": {"status": "not_run_agent_assets_only"},
    }
    ...
    compatibility_path = _generated_root(repo_root) / "guild_asset_sets" / normalized_revision / "guild_asset_set_manifest.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--world-revision",
        default="",
        help="Expected provenance world revision; independent of the preferred asset revision.",
    )
...
def main() -> None:
    args = parse_args()
    if args.allow_procedural_fill:
        raise SystemExit(
            "--allow-procedural-fill is disabled: the strict live-ready feed never "
            "substitutes generated fallback sprites."
        )
    ...
    expected_world_revision = str(args.world_revision or "").strip()
    ...
    required_ready_count = min(target, len(selected_agent_ids))
    strict_ready = (
        not remaining_missing
        and len(assets) >= required_ready_count
        and canonical_manifest is not None
        and str(canonical_manifest.get("status", "")).strip().lower() == "ok"
    )
    print(
        json.dumps(
            {
                "status": "ok" if strict_ready else "failed",
                "target_ready_count": target,
